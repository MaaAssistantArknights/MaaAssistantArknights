...
public class MallSettingsUserControlModel : TaskViewModel
{
    ...
    public override bool? SerializeTask(BaseTask baseTask, int? taskId = null)
    {
        ...
        var fightStage = ConfigFactory.CurrentConfig.TaskQueue.Where(x => x is FightTask).FirstOrDefault()?.IsEnable is not false && ConfigFactory.CurrentConfig.TaskQueue.Where(x => x is FightTask).FirstOrDefault()?.Stage1 == string.Empty;
        if (fightStage)
        {
