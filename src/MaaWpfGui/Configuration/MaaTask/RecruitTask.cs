...
namespace MaaWpfGui.Configuration.MaaTask
{
    ...
    public class RecruitTask : BaseTask
    {
        ...
        /// </summary>
        public string Level3FirstTags { get; set; } = string.Empty;
        ...
        {
            var firstList = Level3FirstTags.Split(';', '；').Select(s => s.Trim());
            ...
            if (Instances.SettingsViewModel.IsLevel3UseShortTime)
            {
            ...
            else if (Instances.SettingsViewModel.IsLevel3UseShortTime2)
            {
                taskParams["recruitment_time"] = new JObject
                {
