...
public:
    ...
    virtual ~InfrastFacilityAnalyzer() override = default;
    InfrastFacilityAnalyzer(const cv::Mat& image, const Rect& roi) = delete;

    bool analyze();

    void set_to_be_analyzed(std::vector<std::string> facilities) noexcept { m_to_be_analyzed = std::move(facilities); }
