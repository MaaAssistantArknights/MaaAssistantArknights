...
void Hasher::set_mask_range(int lower, int upper) noexcept
{
    m_mask_range = std::make_pair(lower, upper);
}

void Hasher::set_mask_range(std::pair<int, int> mask_range) noexcept
{
    m_mask_range = std::move(mask_range);
}

void Hasher::set_hash_templates(std::unordered_map<std::string, std::string> hash_templates) noexcept
{
    m_hash_templates = std::move(hash_templates);
}

void Hasher::set_need_split(bool need_split) noexcept
{
    m_need_split = need_split;
}

void Hasher::set_need_bound(bool need_bound) noexcept
{
    m_need_bound = need_bound;
}

const std::vector<std::string>& Hasher::get_min_dist_name() const noexcept
{
    return m_min_dist_name;
}

const std::vector<std::string>& Hasher::get_hash() const noexcept
{
    return m_hash_result;
}

std::string Hasher::s_hash(const cv::Mat& img)
{
...
std::vector<cv::Mat> Hasher::split_bin(const cv::Mat& bin)
{
...
cv::Mat Hasher::bound_bin(const cv::Mat& bin)
{
    return bin(cv::boundingRect(bin));
}

int Hasher::hamming(std::string hash1, std::string hash2)
{
