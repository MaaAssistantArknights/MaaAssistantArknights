#include "BattlefieldImageClassifier.h"
...
const asst::BattlefieldImageClassifier::ResultOpt& asst::BattlefieldImageClassifier::analyze()
{
    m_result = Result { .object_of_interest = m_object_of_interest };

    if (m_object_of_interest.skill_ready) {
        m_result->skill_ready = skill_ready_analyze();
    }

    if (m_object_of_interest.deploy_direction) {
        m_result->deploy_direction = deploy_direction_analyze();
    }

    return m_result;
}

asst::BattlefieldImageClassifier::SkillReadyResult asst::BattlefieldImageClassifier::skill_ready_analyze()
{
    ...
    float score = std::max(raw_results[0], raw_results[1]);

#ifdef ASST_DEBUG
    if (ready) {
        cv::rectangle(m_image_draw, make_rect<cv::Rect>(m_roi), cv::Scalar(0, 165, 255), 2);
        cv::putText(m_image_draw, std::to_string(score), cv::Point(m_roi.x, m_roi.y - 10), 1, 1.2,
...
asst::BattlefieldImageClassifier::DeployDirectionResult asst::BattlefieldImageClassifier::deploy_direction_analyze()
{
    ...
    cv::putText(m_image_draw, ClassNames.at(class_id), cv::Point(m_roi.x, m_roi.y + m_roi.height),
                cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0, 255, 0), 2);
    cv::putText(m_image_draw, std::to_string(prob[class_id]), cv::Point(m_roi.x, m_roi.y + m_roi.height + 20),
                cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0, 255, 0), 2);
#endif
