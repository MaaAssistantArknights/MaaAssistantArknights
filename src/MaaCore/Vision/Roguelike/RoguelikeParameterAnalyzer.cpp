...
int asst::RoguelikeParameterAnalyzer::update_hope(const cv::Mat& image, const std::string& theme)
{
    ...
    auto roi = Rect(
        ...
        task_roi.height);
    // Clamp ROI to image bounds to guard against out-of-bounds when template matching is slightly off
    roi.x = std::max(0, roi.x);
    roi.y = std::max(0, roi.y);
    roi.width = std::min(roi.width, image.cols - roi.x);
    roi.height = std::min(roi.height, image.rows - roi.y);
    if (roi.width <= 0 || roi.height <= 0) {
        Log.error(__FUNCTION__, "| Computed HopeRecognition ROI is outside image bounds");
        return -1;
    }
    ...
    return utils::chars_to_number(res_vec_opt->front().text, hope_val) ? hope_val : -1;
}
...
int asst::RoguelikeParameterAnalyzer::update_ingot(const cv::Mat& image, const std::string& theme)
{
    ...
    // 通过匹配 IngotFlag 模板确认当前界面状态正确（源石锭资源栏可见）
    Matcher matcher(image);
    ...
    return utils::chars_to_number(res_vec_opt->front().text, ingot_val) ? ingot_val : -1;
}
