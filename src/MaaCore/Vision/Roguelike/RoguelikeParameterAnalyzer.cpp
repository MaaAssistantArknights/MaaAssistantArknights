...
int asst::RoguelikeParameterAnalyzer::update_hope(const cv::Mat& image)
{
    LogTraceFunction;

    // 先识别 当前◀希望◀希望上限 那两个三角形标志
    asst::Matcher matcher_left(image);
    matcher_left.set_task_info(m_theme + "@Roguelike@HopeLeftFlag");
    ...
    matcher_right.set_task_info(m_theme + "@Roguelike@HopeRightFlag");
    ...
    analyzer.set_task_info(m_theme + "@Roguelike@HopeRecognition");
    auto task_roi = Task.get<OcrTaskInfo>(m_theme + "@Roguelike@HopeRecognition")->roi;
    // roi即为两个标志之间的区域
    auto roi = Rect(
...
int asst::RoguelikeParameterAnalyzer::update_ingot(const cv::Mat& image)
{
    LogTraceFunction;

    Matcher matcher(image);
    matcher.set_task_info(m_theme + "@Roguelike@IngotFlag");
    ...
    int ingot_val;
    asst::OCRer analyzer(image);
    analyzer.set_task_info(m_theme + "@Roguelike@IngotRecognition");
...
int asst::RoguelikeParameterAnalyzer::update_ticket_count(const cv::Mat& image)
{
    if (m_theme != "JieGarden") {
        Log.error(__FUNCTION__, "| update_ticket_count only support JieGarden theme");
        return -1;
    }
    return get_number(image, "JieGarden@Roguelike@TicketRecognition");
}
