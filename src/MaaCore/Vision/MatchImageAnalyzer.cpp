...
std::optional<asst::MatchImageAnalyzer::Result> asst::MatchImageAnalyzer::analyze() const
{
    ...
    if (templ.cols > roi.cols || templ.rows > roi.rows) {
        Log.error("templ size is too large", m_templ_name, "roi size:", roi.cols, roi.rows,
    ...
    if (m_mask_range.first == 0 && m_mask_range.second == 0) {
        cv::matchTemplate(roi, templ, matched, cv::TM_CCOEFF_NORMED);
    }
    else {
        cv::Mat mask;
        cv::cvtColor(m_mask_with_src ? roi : templ, mask, cv::COLOR_BGR2GRAY);
        cv::inRange(mask, m_mask_range.first, m_mask_range.second, mask);
        if (m_mask_with_close) {
            cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3));
            cv::morphologyEx(mask, mask, cv::MORPH_CLOSE, kernel);
        }
        cv::matchTemplate(roi, templ, matched, cv::TM_CCOEFF_NORMED, mask);
    }
    ...
    if (m_log_tracing && max_val > m_templ_thres * 0.7) { // 得分太低的肯定不对，没必要打印
        Log.trace("match_templ |", m_templ_name, "score:", max_val, "rect:", rect, "m_roi:", m_roi);
    }
