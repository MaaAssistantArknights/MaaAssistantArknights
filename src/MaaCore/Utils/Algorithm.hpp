 ...
 *                      "干员2"\n
 *                 }
 * @return 可行的分配方案, key 为组名, value 为该组分配的干员, 若无可行方案则返回 std::nullopt,
 * 如:\n
 *         {\n
 *              "A": "干员1",\n
 *              "B": "干员2"\n
 *         }
 */
inline static std::optional<std::unordered_map<std::string, std::string>>
        ...
        const std::unordered_set<std::string>& char_set)
{
    ...
    for (const auto& [group, chars] : group_list) {
        group_name_mapping[group] = group_id_mapping.size();
        group_id_mapping.emplace_back(group);
        bool is_empty = true;

        for (const auto& ch :
             chars | views::filter([&](const auto& c) { return char_set.contains(c); })) {
            is_empty = false;
            node_id_mapping.emplace_back(group, ch);
            if (!char_name_mapping.contains(ch)) {
                char_name_mapping[ch] = char_id_mapping.size();
                char_id_mapping.emplace_back(ch);
            }
        }
    ...
    DancingLinksModel dancing_links_model(
        2 * node_num + group_num + 2 * char_num + 1,
        group_num + char_num);
    ...
    for (size_t i = 0; i < node_num; i++) {
        ...
        dancing_links_model.insert(
            i + 1,
            group_num + char_name_mapping[node_id_mapping[i].second] + 1);
    }
    ...
    // dance!!
    const bool has_solution = dancing_links_model.dance(1);
