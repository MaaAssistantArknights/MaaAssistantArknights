...
bool asst::SupportList::select_role(const Role role)
{
    ...
    if (m_in_support_unit_detail_panel) {
        Log.error(__FUNCTION__, "| Attempting to select role in support unit detail panel");
        return false;
    }
...
bool asst::SupportList::update()
{
    ...
    if (m_in_support_unit_detail_panel) {
        Log.error(__FUNCTION__, "| Attempting to update support list in support unit detail panel");
        return false;
    }
...
bool asst::SupportList::select_support_unit(const size_t index)
{
    ...
    if (m_in_support_unit_detail_panel) {
        Log.error(__FUNCTION__, "| Attempting to select support unit in support unit detail panel");
        return false;
    }
...
bool asst::SupportList::use_support_unit()
{
    LogTraceFunction;

    if (!m_in_support_unit_detail_panel) {
        Log.warn(__FUNCTION__, "| Currently not in support unit detail panel");
        return false;
    }

    if (ProcessTask(m_task, { "SupportList-Confirm" }).run()) {
    ...
    if (skill != 0) {
        if (const int rarity = BattleData.get_rarity(m_list[index].name); max_spec_lvl && rarity >= 3) {
            ...
            if (!max_spec_lvl_analyzer.analyze()) { // 所需技能非专三/7级
                // 取消选择
                leave_support_unit_detail_panel();
                return false;
            }
        }
        // 选择技能
...
bool asst::SupportList::refresh_list()
{
    ...
    if (m_in_support_unit_detail_panel) {
        Log.error(__FUNCTION__, "| Attempting to refresh support list in support unit detail panel");
        return false;
    }
...
bool asst::SupportList::update_selected_role(const cv::Mat& image)
{
    ...
    if (m_in_support_unit_detail_panel) {
        Log.error(__FUNCTION__, "| Attempting to update selected role in support unit detail panel");
        return false;
    }
...
std::optional<unsigned> asst::SupportList::update_page()
{
    ...
    // 我们假设 support_list_page 为一段从左往右连续编号的助战栏位序列，且所有已知助战栏位在前、新助战栏位在后
    // 设置 support_list_page 的起点索引以供检查
    ...
    for (const SupportUnit& support_unit : support_list_page) {
        ...
        if (const auto it = m_name2index.find(support_unit.name); it == m_name2index.end()) // 新助战栏位
        {
            ...
            if (index != m_list.size()) [[unlikely]] {
                // 已知的助战栏位中间夹了个新的，即 ...OLD, NEW, OLD...，可能是前一页助战列表页识别出错
            ...
            if (index != it->second) [[unlikely]] {
                // 新的助战栏位中间夹了个已知的，即 ...NEW, OLD, NEW...，可能是前一页助战列表页识别出错
        ...
        // update view
        // ————————————————————————————————————————————————————————————————
        // index 只会递增，因而无需考虑 m_view_begin
