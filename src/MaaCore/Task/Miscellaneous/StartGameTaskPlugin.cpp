...
{
    if (m_client_type.empty()) {
        LogError << __FUNCTION__ << "| client_type is empty, cannot restart game";
        return false;
    }
    if (!ctrler()) {
        LogError << __FUNCTION__ << "| controller is not initialized, cannot restart game";
        return false;
    }
    LogInfo << __FUNCTION__ << "| stopping game client";
    ctrler()->stop_game(m_client_type);
    sleep(3000);
    LogInfo << __FUNCTION__ << "| starting game client";
    return _run();
}
