...
bool asst::RoguelikeDifficultySelectionTaskPlugin::_run()
{
    ...
    if (m_roguelike_data->get_theme() != "Phantom" && mode == "4") {
        if (difficulty == "max") {
            ...
            ProcessTask(*this, { m_roguelike_data->get_theme() + "@Roguelike@ChooseDifficulty_Easiest" }).run();
        }
        ProcessTask(*this, { m_roguelike_data->get_theme() + "@Roguelike@ChooseDifficultyConfirm" }).run();
    }
