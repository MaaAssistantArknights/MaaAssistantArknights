    ...
    [[maybe_unused]] std::optional<std::reference_wrapper<cv::Mat>> image_draw_opt)
{
    ...
    for (size_t prev = pre_col_begin; prev < pre_col_end; ++prev) {
        ...
        auto leftmostBrightPixels = brightPixels | std::views::filter([&](const Point& p) { return p.x == leftmost_x; });
        auto rightmostBrightPixels = brightPixels | std::views::filter([&](const Point& p) { return p.x == rightmost_x; });
