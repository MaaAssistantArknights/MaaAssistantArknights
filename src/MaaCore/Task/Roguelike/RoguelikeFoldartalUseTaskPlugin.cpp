...
bool asst::RoguelikeFoldartalUseTaskPlugin::verify(AsstMsg msg, const json::value& details) const
{
    ...
    if (m_roguelike_data->get_theme() != "Sami") {
        return false;
    }
    std::string mode = status()->get_properties(Status::RoguelikeMode).value();
    std::string task_name_pre = m_roguelike_data->get_theme() + "@Roguelike@Stage";
...
bool asst::RoguelikeFoldartalUseTaskPlugin::_run()
{
    ...
    std::vector<RoguelikeFoldartalCombination> combination =
        RoguelikeFoldartal.get_combination(m_roguelike_data->get_theme());
...
bool asst::RoguelikeFoldartalUseTaskPlugin::use_board(const std::string& up_board, const std::string& down_board)
{
    ...
    if (ProcessTask(*this, { m_roguelike_data->get_theme() + "@Roguelike@Foldartal" }).run()) {
        swipe_to_top();
        ...
        if (search_and_click_board(up_board) && search_and_click_board(down_board)) {
            ...
            if (ProcessTask(*this, { m_roguelike_data->get_theme() + "@Roguelike@FoldartalUseConfirm" }).run()) {
                return true;
            }
        }
        ProcessTask(*this, { m_roguelike_data->get_theme() + "@Roguelike@FoldartalBack" }).run();
    }
    return false;
}
...
bool asst::RoguelikeFoldartalUseTaskPlugin::search_and_click_board(const std::string& board)
{
    ...
    while (try_time < max_retry && !need_exit()) {
        ...
        std::string task_name = m_roguelike_data->get_theme() + "@Roguelike@FoldartalUseOcr";
...
bool asst::RoguelikeFoldartalUseTaskPlugin::search_and_click_stage()
{
    ...
    // 节点会闪烁，所以这里不用单次Match
    if (ProcessTask(*this, { m_roguelike_data->get_theme() + "@Roguelike@FoldartalUseOnStage" }).run()) {
        return true;
    }
...
void asst::RoguelikeFoldartalUseTaskPlugin::swipe_to_top()
{
    ...
    while (try_time < max_retry && !need_exit()) {
        while (try_time < max_retry && !need_exit()) {
            OCRer analyzer(ctrler()->get_image());
            analyzer.set_task_info(m_roguelike_data->get_theme() + "@Roguelike@FoldartalUseOcr");
            if (analyzer.analyze()) {
                return;
            }
            else {
