...
bool BlackFlowLifecycleTaskPlugin::load_params(const json::value& params)
{
    ...
    if (is_baby_animal_profile(profile)) {
        ...
        if (!target.has_value()) {
            LogError << "Invalid BlackFlow cultivation target:" << target_text;
            return false;
        }
    ...
    if (!m_session->initialize(std::move(profile), &error)) {
        LogError << "BlackFlow strategy initialization failed:" << error;
        return false;
    }
    ...
    if (!diagnostics.has_value() || image_limit < 0 || image_limit > 100) {
        LogError << "Invalid BlackFlow diagnostics parameters";
        return false;
    }
    ...
    if (!m_session->configure_diagnostics(settings, &error)) {
        LogError << "BlackFlow diagnostics initialization failed:" << error;
        return false;
    }
    if (m_port != nullptr) {
    ...
    LogInfo << "BlackFlow strategy initialized" << "profile" << selected_profile;
...
bool BlackFlowLifecycleTaskPlugin::_run()
{
    ...
    if (work == PendingWork::RecordCurrentFloor) {
        if (m_session == nullptr) {
            LogError << "BlackFlow floor recognition has no active session";
            return false;
        }
        ...
        if (task == nullptr || area_name.empty()) {
            ...
            LogError << "BlackFlow NextLevel floor recognition result is missing";
            report_outputs();
            return true;
        }
        ...
        if (matched == floor_names.end()) {
            ...
            m_session->fail(
                ...
                FailureDisposition::RestartRun);
            LogError << "BlackFlow NextLevel returned an unconfigured floor name" << area_name;
            report_outputs();
            return true;
        }
        ...
        if (!m_session->set_current_floor(floor, &error)) {
            ...
            LogError << "BlackFlow NextLevel floor recognition failed" << area_name << error;
            report_outputs();
            return true;
        }
        LogInfo << "BlackFlow current floor recognized" << "floor" << floor << "area" << area_name;
        report_outputs();
        return true;
    }
    if (work == PendingWork::ResetAfterAbandon) {
        ...
        LogInfo << "BlackFlow run state reset after abandonment";
        if (stop_after_abandon && m_task_ptr != nullptr) {
            m_task_ptr->set_enable(false);
            LogInfo << "BlackFlow task stopped after abandonment";
        }
        return true;
    }
    ...
    if (m_session != nullptr && !m_session->terminated()) {
        ...
        LogError << "BlackFlow terminated without a strategy result" << trigger << pre_task << outcome << reason;
    ...
    LogInfo << "BlackFlow strategy terminal action" << "profile" << profile << trigger << pre_task << outcome << reason
            << next_action << task;
    report_outputs();
    return true;
}
