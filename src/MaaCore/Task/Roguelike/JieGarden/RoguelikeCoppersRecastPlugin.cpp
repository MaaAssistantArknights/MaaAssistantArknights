...
std::optional<int> resolve_metric(
    asst::RoguelikeCoppersRecastCondition::ConditionType type,
    const asst::RoguelikeCoppersRecastSnapshot& snapshot)
{
    using ConditionType = asst::RoguelikeCoppersRecastCondition::ConditionType;
    switch (type) {
...
bool asst::RoguelikeCoppersRecastCondition::evaluate(const RoguelikeCoppersRecastSnapshot& snapshot) const
{
...
{
using ConditionType = asst::RoguelikeCoppersRecastCondition::ConditionType;
using CompareType = asst::RoguelikeCoppersRecastCondition::CompareType;
...
bool asst::RoguelikeCoppersRecastPlugin::configure(const json::value& payload)
{
    std::vector<RoguelikeCoppersRecastCondition> conditions;

    if (auto array_opt = payload.find<json::array>("conditions")) {
...
void
    asst::RoguelikeCoppersRecastPlugin::set_conditions(std::vector<RoguelikeCoppersRecastCondition> conditions) noexcept
{
...
bool asst::RoguelikeCoppersRecastPlugin::_run()
{
    ...
    RoguelikeParameterAnalyzer parameter_analyzer("JieGarden");
    ...
    while (true) {
        ...
        const int hp_val = parameter_analyzer.update_hp(image);
        m_snapshot.hp = hp_val >= 0 ? std::optional<int>(hp_val) : std::nullopt;

        const int hope_val = parameter_analyzer.update_hope(image);
        m_snapshot.hope = hope_val >= 0 ? std::optional<int>(hope_val) : std::nullopt;

        const int ingot_val = parameter_analyzer.update_ingot(image);
        m_snapshot.ingot = ingot_val >= 0 ? std::optional<int>(ingot_val) : std::nullopt;

        const int ticket_val = parameter_analyzer.update_ticket_count(image);
