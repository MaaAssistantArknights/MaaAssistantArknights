...
void asst::RoguelikeFormationTaskPlugin::clear_and_reselect()
{
    ...
    const auto& team_complete_condition = RoguelikeRecruit.get_team_complete_info(m_roguelike_data->get_theme());
    const auto& group_list = RoguelikeRecruit.get_group_info(m_roguelike_data->get_theme());
    for (const auto& condition : team_complete_condition) { // 优先选择阵容核心干员
        ...
        for (const std::string& group_name : condition.groups) {
            for (const auto& oper : oper_list) {
                std::vector<int> group_ids = RoguelikeRecruit.get_group_id(m_roguelike_data->get_theme(), oper.name);
