...
{
    ProcessTask(*this, { m_roguelike_data->get_theme() + "@Roguelike@WaitForStartButtonClicked" })
        .set_task_delay(0)
        .set_retry_times(0)
        .run();
}
...
bool asst::RoguelikeBattleTaskPlugin::do_best_deploy()
{
    ...
    const auto& groups = RoguelikeRecruit.get_group_info(m_roguelike_data->get_theme());
    // 获取当前肉鸽的分组内排名信息
    ...
    for (const auto& [name, oper] : m_cur_deployment_opers) {
        ...
        // 获取招募信息
        const auto& recruit_info = RoguelikeRecruit.get_oper_info(m_roguelike_data->get_theme(), oper_name);
        // 获取会用到该干员的干员组[干员组1序号,干员组2序号,...]
        std::vector<int> group_ids = RoguelikeRecruit.get_group_id(m_roguelike_data->get_theme(), oper_name);
    ...
    for (const auto& deploy_plan : deploy_plan_list) {
        if (!m_used_tiles.contains(deploy_plan.placed) &&
            ...
            const auto& oper_info =
                RoguelikeRecruit.get_oper_info(m_roguelike_data->get_theme(), deploy_plan.oper_name);
...
bool asst::RoguelikeBattleTaskPlugin::do_once()
{
    ...
    for (const auto& [name, loc] : pre_battlefield) {
        const auto& oper_info = RoguelikeRecruit.get_oper_info(m_roguelike_data->get_theme(), name);
    ...
    if (do_best_deploy()) { // 这是新的部署逻辑，更加精确
        ...
        // 获取技能用法和使用次数
        const auto& oper_info = RoguelikeRecruit.get_oper_info(m_roguelike_data->get_theme(), best_oper.name);
