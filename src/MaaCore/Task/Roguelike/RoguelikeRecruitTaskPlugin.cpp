...
bool asst::RoguelikeRecruitTaskPlugin::is_oper_melee(const std::string& name)
{
    ...
    if (role != battle::Role::Pioneer && role != battle::Role::Tank
        && role != battle::Role::Warrior) {
        return false;
    }
...
bool asst::RoguelikeRecruitTaskPlugin::_run()
{
    ...
    m_config->set_recruitment_starts_complete(
    ...
    for (; i != SwipeTimes; ++i) {
        ...
        for (const auto& oper_info : oper_list) {
            ...
            // 查询招募配置
            const auto& recruit_info =
                RoguelikeRecruit.get_oper_info(m_config->get_theme(), oper_info.name);
            int priority = 0;

            // 查询编队是否已持有该干员
            if (const auto& char_opt = chars_map.find(oper_info.name);
                char_opt != chars_map.end()) {
                ...
                if (recruit_info.is_alternate) {
                    ...
                    // 干员待晋升
                    priority = team_full_without_rookie
                ...
                if (oper_info.elite == 2) {
                    ...
                    priority = team_full_without_rookie
                                   ? recruit_info.recruit_priority_when_team_full
                                         + recruit_info.promote_priority_when_team_full
                    ...
                    // 精一50级以上
                    // 等级是 0
                    // 一般是识别错了，多发生于外服，一般都是一错全错，先凑合着招一个吧，比不招人强
                    // orz
                    priority = team_full_without_rookie
                                   ? recruit_info.recruit_priority_when_team_full
                ...
                if (!recruit_info.is_alternate) {
                    ...
                    int role_num =
                        recruit_info.offset_melee ? offset_melee_num : team_roles[oper_role];
                    for (const auto& offset_pair :
                         ranges::reverse_view(recruit_info.recruit_priority_offset)) {
                    ...
                    // const auto role_info = RoguelikeRecruit.get_role_info(rogue_theme,
                    // oper_role); for (const auto& offset_pair : ranges::reverse_view(role_info)) {
                    //     if (role_num >= offset_pair.first) {
                    //         priority += offset_pair.second;
                    //         break;
                    //     }
            ...
            // 添加到候选名单
            auto existing_it =
                ranges::find_if(recruit_list, [&](const RoguelikeRecruitInfo& pri) -> bool {
    ...
    if (recruit_list.empty()) {
        ...
        //         continue;
        //     }
        //     Log.trace(__FUNCTION__, "| Choose random elite 1:", info.name, info.elite,
        //     info.level); recruit_oper(info); return true;
        // }
    ...
    // 选择优先级最高的干员
    auto selected_oper = ranges::max_element(
        recruit_list,
        std::less {},
        std::mem_fn(&RoguelikeRecruitInfo::priority));
...
bool asst::RoguelikeRecruitTaskPlugin::recruit_appointed_char(
    ...
    bool is_rtl)
{
    ...
    for (; i != SwipeTimes; ++i) {
        ...
        if (analyzer.analyze()) {
            ...
            max_oper_x =
                ranges::max(chars | views::transform([&](const auto& x) { return x.rect.x; }));
            auto it =
                ranges::find_if(chars, [&](const battle::roguelike::Recruitment& oper) -> bool {
            ...
            if (pre_oper_names == oper_names) {
                if (has_been_same) {
                    Log.trace(
                        __FUNCTION__,
                        "| Oper list not changed for three times, stop swiping");
                    break;
                }
                else {
                    has_been_same = true;
...
{
    std::string swipe_task_name = to_left ? "RoguelikeRecruitOperListSlowlySwipeToTheLeft"
    ...
    ctrler()->swipe(
        StartPoint,
        { StartPoint.x + swipe_dist - StartPoint.width,
          StartPoint.y,
          StartPoint.width,
          StartPoint.height },
