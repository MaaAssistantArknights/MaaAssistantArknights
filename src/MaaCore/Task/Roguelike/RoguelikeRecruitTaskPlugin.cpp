...
bool asst::RoguelikeRecruitTaskPlugin::_run()
{
    ...
    auto theme = m_roguelike_data->get_theme();
    ...
    const auto& group_list = RoguelikeRecruit.get_group_info(theme);
    for (const auto& oper : chars_map) {
        std::vector<int> group_ids = RoguelikeRecruit.get_group_id(theme, oper.first);
    ...
    if (!start_complete) {
        for (const auto& oper : chars_map) {
            auto& recruit_info = RoguelikeRecruit.get_oper_info(theme, oper.first);
    ...
    if (!team_complete) {
        ...
        const auto& team_complete_condition = RoguelikeRecruit.get_team_complete_info(theme);
    ...
    for (; i != SwipeTimes; ++i) {
        ...
        for (const auto& oper_info : oper_list) {
            ...
            // 查询招募配置
            auto& recruit_info = RoguelikeRecruit.get_oper_info(theme, oper_info.name);
            int priority = 0;
            ...
            if (it != chars.cend()) {
                ...
                if (start_with_elite_two == "1" && difficulty == "max") {
                    if (it->elite == 2) {
                        ...
                        ProcessTask(*this, { m_roguelike_data->get_theme() + "@Roguelike@ExitThenAbandon" })
