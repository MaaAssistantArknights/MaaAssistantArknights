...
bool asst::CopilotTask::set_params(const json::value& params)
{
    ...
    std::unordered_set<std::string> prechecked_missing_opers;
    if (auto opt = params.find<json::array>("prechecked_missing_opers")) {
        for (const auto& oper : *opt) {
            prechecked_missing_opers.emplace(oper.as_string());
        }
    }
    ...
    m_formation_task_ptr->set_prechecked_missing_opers(std::move(prechecked_missing_opers));
