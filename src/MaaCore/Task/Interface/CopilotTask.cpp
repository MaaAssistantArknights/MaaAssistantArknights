...
bool asst::CopilotTask::set_params(const json::value& params)
{
    ...
    if (filename_opt) {
        ...
        if (Copilot.get_data().info.is_sandbox) {
            ...
            std::vector<std::string> sandbox_steps = {
                "SandboxOption1_" + std::to_string(opt1),
        ...
        for (const auto& [filename, stage_name, is_raid, is_sandbox, sandbox_option1, sandbox_option2] : configs) {
