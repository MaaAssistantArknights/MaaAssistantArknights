...
namespace asst {
	...
	protected:
		...
		constexpr static int MaxOperNumber = 3;		// 单个制造站/贸易站最大干员数
		constexpr static int MaxStationNumber = 5;	// 设施最大数量（基建里最多只有5个制造站）
		...
		// 一边滑动一边识别并点击干员名
		bool swipe_and_select(const std::vector<std::string>& name_comb, int swipe_max_times = SwipeMaxTimes);
