...
class Bluestacks:
    ...
    def get_hyperv_port(
        ...
        instance_name="Pie64",
        read_imageinfo_from_config=False,
    ) -> int:
        ...
        with open(conf_path, encoding="UTF-8") as f:
            ...
            if read_imageinfo_from_config:
                instances = [
                    i.strip('"') for i in configs["bst.installed_images"].split(",")
                ]
                instance_name = instances[0]
        return int(
            configs[f"bst.instance.{instance_name}.status.adb_port"].replace('"', "")
        )

    @staticmethod
    def launch_emulator_win(
