...
* MAA updater 增加文件被占用时增加重试、互斥量协调与回滚机制 (#16703) @soundofautumn
...
* 不再使用不必要的子任务 @status102
* OCRer DEBUG下m_image_draw绘制增加结果文本 @status102

### 修复 | Fix

* 界园移时换物放弃探索 @Saratoga-Official
* 水月肉鸽事件DiceConfirm后事件不处理 @status102
* respect timeout while waiting for macOS socket data (#17027) @Halo5082
* 15 章之后的难度切换 @ABA2396
...
* build warning @ABA2396
* 自动战斗导航选错关卡无法重选 @status102
* 多作业模式关卡名复核失效 @status102
* 处理界园事件内通宝交换后的事件结束页 (#16936) @ZiyinLin
* 有小猪 @status102
...
* Merge commit from fork @ABA2396
* 更新肉鸽招募干员 @Saratoga-Official
* 移除未使用的变量 @ABA2396
* 简化 external_renderer_ipc 加载成功的日志 @ABA2396
* Release v6.12.1 (#17078) @ABA2396
