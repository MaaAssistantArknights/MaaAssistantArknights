...
* 新增「库存保持」任务，支持在单个任务中添加多组保持计划，按优先级将指定材料刷取至目标数量；可在任务开始前更新库存，支持 SideStory 活动与资源关卡限时全开放期间自动跳过，并支持 AUTO 代理倍率（游戏更新后倍战锁定期间同步禁用） ([#17042](https://github.com/MaaAssistantArknights/MaaAssistantArknights/pull/17042)) @status102 @ABA2396
...
* 进入基建时增加专用加载页识别，减少加载过程中状态误判 ([#17434](https://github.com/MaaAssistantArknights/MaaAssistantArknights/pull/17434)) @wzacolemak
* 优化库存保持任务的显示效果与 Item 绑定性能 @status102 @ABA2396
...
* 修复 WPF 配置迁移引入的 `client_type` 参数序列化错误（枚举值被序列化为数字而非字符串）及外服游戏客户端类型迁移失败，导致开始唤醒、战斗、关闭客户端等任务添加失败的问题 @status102
* 修复库存保持任务配置未能正确触发保存、Plan 删除后 PropertyChanged 事件泄漏及 id 持久化问题 @status102 @ABA2396
* 修复 Discord Webhook 预置模板及配置迁移时 Body 格式错误的问题 @status102
