...
def find_missing_templates(server_name):
    ...
    zh_files = [
        f for f in os.listdir(zh_dir) if os.path.isfile(os.path.join(zh_dir, f))
    ]
    gl_files = [
        f for f in os.listdir(gl_dir) if os.path.isfile(os.path.join(gl_dir, f))
    ]
    ...
    diff_files = [
        f
        for f in zh_files
        if not any(map(lambda x: re.search(x, f), regex_ignore_list))
        and f not in gl_files
