...
def export_model(model: ExtraTreesClassifier, output_dir: Path, labels: list[str], feature_count: int, report: dict) -> None:
    ...
    (output_dir / "DrowningSeekers_node_type.onnx").write_bytes(onnx.SerializeToString())
    ...
    (output_dir / "README.txt").write_text(
        ...
        "The model is consumed by the C++ analyzer as DrowningSeekers_node_type.\n",
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    try:
        map_data = json.loads((repo_root / "resource" / "roguelike" / "DrowningSeekers" / "map.json").read_text(encoding="utf-8"))
    ...
    print(f"saved: {args.output / 'DrowningSeekers_node_type.onnx'}")
    return 0
