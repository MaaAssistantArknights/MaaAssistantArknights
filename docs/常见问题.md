...
- 可能性 1: 运行库问题。  
  ...
  * 如果您是Windows Server或其他精简版系统的用户，在安装上述运行环境后如果仍无法运行，可尝试使用 [Microsoft C++ 生成工具](https://visualstudio.microsoft.com/zh-hans/visual-cpp-build-tools/) 进行完整的开发环境配置（您仅需要安装.net及c++开发环境）。**请注意，此项操作将会耗费您10G左右的磁盘空间，请务必在排查完毕其他可能性后使用。**
- 可能性 2: CPU 指令集不支持。  
