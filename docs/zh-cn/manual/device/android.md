...
如果你遇到这个问题，可以尝试手动更改 `开发者选项→最小宽度`，或使用指令 `adb shell wm density <值>` 。
**注意该操作存在风险可能使设备工作异常，请确保你知道自己在做什么，设备故障后果自负。**建议取值为 `[100-800]`。
强烈建议在**下次重启设备前**还原该值，可使用指令 `adb shell wm density reset`。
:::
