...
- `使用药剂` + `使用源石` 和 `指定次数`、`指定材料` 三个选项为短路开关（或门），即达成三个选项中的任一条件，均会视为任务完成，停止理智作战。
  ...
  - `指定材料` 指定刷多少个指定材料（例如“获取 5 个固源岩后停止”），支持两种计数模式：
    ...
    - `目标库存`：参考 [仓库识别](./tools.md#仓库识别) 中保存的库存数据，只补到设定库存为止。需要先通过 [更新数据](./user-data-update.md) 或 [仓库识别](./tools.md#仓库识别) 获取库存。此模式由 UI 实现（MaaCore 仅支持掉落数量模式，UI 根据库存缓存计算缺口后转换为掉落数量）。
