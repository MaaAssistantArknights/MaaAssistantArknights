...
- 高级设置中的 `剿灭仅使用全权委托` 默认关闭。关闭时保持原有行为：优先尝试全权委托，全权委托不可用但普通代理可用时会继续执行普通代理；普通代理也不可用时任务失败。
- 启用 `剿灭仅使用全权委托` 后，每次开始剿灭前都会检查是否可用全权委托。若代理券已耗尽或全权委托不可用，当前理智作战会正常结束，并继续执行下一个任务，不会回退到普通代理。
