...
::: info UI 专属功能
本页部分功能由 UI 层实现（如目标库存、智能跳过、周计划、多任务排序等），详见[新手上路](../newbie.md#关于本文档)中的说明。
:::
...
- 「根据活动排期智能跳过」与「周计划」位于 `任务设置` - `理智作战` - `高级设置`，两个选项独立生效。这些功能由 UI 实现。
- 「根据活动排期智能跳过」：先检查当前游戏日是否有 SideStory 活动；若有，且连续或重叠的后续活动会在本周结束前终止，则跳过当前理智作战任务，以便在活动结束后的任意一天执行剿灭作战。如果 SideStory 活动连续覆盖本周剩余时间直至周日结束，则不会跳过。
- 启用「周计划」后，可勾选周日～周六中哪些**游戏日**执行该理智作战任务。
