...
- 指定次数设为 10, 代理倍率设为 4: 将执行 2 (次开始行动) x 4(倍代理) = 8 次作战(2 x floor(10 / 4) = 8), 消耗 8 x 6 = 48 理智。此时若再进行一次4倍代理则达到 12 次作战, 超过设定的 10 次, 故不再执行, 以 8 次作战结束任务
- 指定次数设为 10, 代理倍率设为 AUTO: 将执行 一次6倍代理 + 一次4倍代理 = 10 次作战(6 * floor(10 / 6) + (10 % 6) = 10), 消耗 10 x 6 = 60 理智
