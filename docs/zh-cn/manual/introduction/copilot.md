...
- 可使用 `多作业模式` 功能进行同一区域关卡的自动连续战斗。
  - 作业列表下方三个按钮从左到右依次为 `批量导入`、`添加`、`清除`。  
    `添加` 左键为添加普通难度关卡，右键为添加突袭难度关卡；`清除` 左键为清除所有关卡，右键为清除未勾选关卡，
  - 导入作业后，作业列表下方会出现关卡名，确认正确后再添加该关卡。列表中的关卡可以拖拽调整顺序，勾选是否执行。
