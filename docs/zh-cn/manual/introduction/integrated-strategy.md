...
MAA 默认选择傀影主题，可在 `自动肉鸽` - `肉鸽主题` 中更改目标主题。
...
- 设置中可选择主题、难度、分队、开局干员等。开局干员默认仅第 1 顺位，可在 `高级设置` 中启用第 2、3 顺位，三个顺位的干员职业不能相同。
- 招募只在前几页内翻找指定干员，希望消耗低的干员可能因排位靠后而找不到，届时该位置会回退默认策略，建议将希望消耗高的干员排在前面。
- MAA 可以使用助战干员开局，需先在 `常规设置` → `开局干员` 中填写目标干员名，再在 `高级设置` 中启用各开局干员对应的使用助战勾选框；每个顺位均可独立设置是否借助战，但第 1 顺位借助战与凹直升精二互斥（凹直升精二仅支持自有干员）。
...
| 黑流树海 | _暂无_ <!-- 保密等级·0~15 --> | 特勤分队 / 堡垒战术分队 | 稳扎稳打 / 坚不可摧 | `机械师`（必须精二并解锁 `招募精通Ⅱ` 的 `效果提升Ⅱ` ） |
...
| 黑流树海 | 自动战斗等策略尚在编写中，相关决策由通用策略决定。~~你们最喜欢的 AI 来了。~~<br>当前策略需要 `机械师` 精二，并解锁 `招募精通Ⅱ` 天赋的 `效果提升Ⅱ`，开局才会携带 `结构性原理` 零件（全图任意飞 3 次）；该天赋解锁难度较低（精二后累计卖出 10 个零件），推荐先行解锁。招募到机械师后为保证效率不再检查精英化状态，直接按精二执行策略，未精二会卡住甚至死循环；且通关一次前部分事件的选项会不同，遇到可能卡死，先通关一次 N1 可同时满足精二与事件选项的前提。<br>若未满足上述前提，则必须使用其他**无召唤物**的强力干员开局，且**仅推荐使用刷源石锭策略**（战斗大概率失败，其他策略待后续优化）。<br>当使用刷等级策略，未勾选 `投资源石锭` 时，将会跳过商店。 |
...
- 战斗时长超过 8 分钟会自动撤退所有地面单位，超过 10 分钟会自动放弃当局战斗，避免互相刮痧耗时。
