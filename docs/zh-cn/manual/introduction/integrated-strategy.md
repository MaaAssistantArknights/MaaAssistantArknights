...
| 黑流树海 | 自动战斗等策略尚在编写中，相关决策由通用策略决定。~~你们最喜欢的 AI 来了。~~<br>当前策略需要 `机械师` 的 `招募精通Ⅱ` 在解锁 `效果提升Ⅱ` 后被进阶时获得的 `结构性原理`。该精通解锁难度较低，推荐先行解锁。仅解锁 `招募精通Ⅱ` 的 `效果提升Ⅰ` 仍然可行，但效率会大幅降低。<br>若未解锁上述 `效果提升` 则必须使用其他**无召唤物**的强力干员开局，且**仅推荐使用刷源石锭策略**。<br>当使用刷等级策略，未勾选 `投资源石锭` 时，将会跳过商店。 |
