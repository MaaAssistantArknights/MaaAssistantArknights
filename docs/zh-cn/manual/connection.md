...
需使用官版或方舟专版 MuMu 12 V4.0.0 及更新版本，或国际版的 MuMu 12 V4.1.26 及更新版本。<!-- 官版 V3.8.13 支持初版截图增强 -->

1. `设置` - `连接设置`，勾选 `启用 MuMu 截图增强模式`，MAA 会在勾选开关时自动尝试通过注册表获取安装路径。

2. `MuMu 安装路径` 填写 `MuMuPlayer-12.0` 或 `YXArkNights-12.0` 或 `MuMuPlayerGlobal-12.0` 文件夹的路径，如 `C:\Program Files\Netease\MuMuPlayer-12.0`。
