...
```json
{
    ...
    "actions": [
        // 战斗中的操作。有序，执行完前一个才会去执行下一个。必选
        {
            ...
            // "设置单位坐标" 设置某战场单位的格子坐标："name" 与 "location" 均必填，任一缺失或值畸形（"name" 需为非空字符串，"location" 需为 [x, y] 两元素数字数组）会导致作业加载失败
            //      仅更新 MAA 内部的单位位置记录，不对游戏进行任何操作
            //      用于干员位移后修正位置，使后续按 "name" 的 "技能" / "撤退" / "技能用法" 动作落到新格子；
            //      也可用于登记场上装置、设施等非干员单位，配合 "技能用法" 与 "摆完挂机" 实现装置技能自动开启；
            //      该 "name" 已在场上时更新其位置，不在场上时登记为新单位（"role" 可选，缺省不区分职业）
            // 目前下面五个条件是且的关系，即 &&
            ...
            //                        // 0 - 且； 1 - 或
            "name": "棘刺", // 干员名 或 群组名， type 为 "部署" 时必选，为 "技能" | "撤退" 时可选，为 "设置单位坐标" 时必选
            "role": "guard", // 干员职业。可选，用于区分同名干员，填英文职业名，大小写不限
            "location": [
                5,
                5
            ...
            // type 为 "设置单位坐标" 时必填，将该 "name" 单位的位置设置为此格子，坐标为任意合法格子，包括 [0, 0]
