...
```json
{
    "TaskName": {                           // 任务名，带 @ 时可能为特殊任务，字段默认值会有不同，详见下方特殊任务类型
        ...
        "cache": false,                    // 可选项，表示该任务是否使用缓存，默认为 false
                                            // 开启后第一次识别到目标时，以后永远只在第一次识别到的位置进行识别，可大幅节省性能
                                            // 但仅适用于待识别目标位置完全不会变的任务，若待识别目标位置会变请保持默认的 false
        ...
        "specialParams": [int, ...],        // 某些特殊识别器需要的参数
                                            // 额外的，当 action 为 Swipe 时可选，[0] 表示 duration，[1] 表示 额外滑动的方向（0 不启用，1/2/3/4 分别为上/下/左/右），
                                            // [2] 和 [3] 分别表示滑动轨迹的缓入、缓出斜率，需要乘 10 输入，默认均为 10
                                            // 如果需要正常进入并缓出，则 [2]、[3] 建议分别为 37, 1
        ...
        "nmsDistance": 0,                   // 可选项，模板匹配多结果去重半径，单位为像素，默认为 0
                                            // 两个命中位置的横纵坐标差都小于该值时，只保留得分最高的一个
                                            // 不填或 0 时按模板短边的一半取值
        ...
        "replaceFull": false,               // 可选项，ocrReplace 命中时是否替换整段识别文字，默认为 false
                                            // false 时仅替换命中的子串；true 时只要整段文字命中任一替换规则，就将整段文字替换为该规则的替换值
