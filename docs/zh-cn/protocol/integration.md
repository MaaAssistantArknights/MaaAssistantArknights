...
- 复刻的 SS 活动。输入 `SSReopen-<关卡前缀>` ，可一次性刷完 XX-1 ~ XX-9 关，如 `SSReopen-IC`。
  ...
  `-1` 为禁用切换。
  <br>
  `0` 为自动切换为当前可用的最大次数, 如当前理智不够最大次数, 则选择最低可用次数。
  <br>
  `1~10` 为指定连战次数。
  <br>
  ::: info 服务器差异
  Core 根据游戏内连战列表样式动态识别可选次数，不硬编码上限。输入验证取决于资源版本：
  <br>
  国服于 2026/8/1 更新后采用新列表样式，接受 `-1~10`；外服仍为旧样式，仅接受 `-1~6`，传入更大的值会被拒绝。外服预计约半年后跟进，届时上限自动生效。
  :::  
  :::  
