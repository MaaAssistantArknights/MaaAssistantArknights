           ...
           "opers": [                                        // 该包括哪些干员，有序，代表部署优先度
               {
                   ...
                   "role": "Warrior",                        // 干员职业，可选。存在同名干员时用于区分职业，填英文职业名，大小写不限
                   ...
                   "is_alternate": false,                    // true 为后备干员（允许重复招募，划到后备干员时不再往右划动），false 或省略为普通干员
                   ...
                   "collection_priority_offsets": [          // 按当前拥有的藏品调控招募优先级
                       {
                           "collection": "古旧的钱币",        // 藏品名称
                           "offset": 100                     // 拥有该藏品后对招募优先级的调整，不写默认 0
                       }
                   ]
               },
               ...
...
{
    "theme": "Sami",                              //肉鸽主题
    "mode": [1, 4],                               //限定该文件在哪些模式下生效，可选；不写则对所有模式生效
    "stage": [                                    //不期而遇类事件
        {
            "name": "低地市集",                    //不期而遇事件名称
            ...
            "choices": [                          //对特定选项的额外要求
                {
                    "name": "选择碎草药",          //选项的名字
                    "choose": 1,                  //满足下方条件时优先选择的选项序号，可选；不写时该条不生效
                    "requirements": [             //生效条件，可选
                        {
                            "name": "Vision",     //条件类型，目前仅支持 Vision（灯火值）
                            "value": "3",         //要求的数字
                            "type": ">"           //是大于还是小于（还可以是 =，这里表示灯火大于 3 时该条 choose 才生效）
                        }
                    ]
                },
                {
...
- `option_text`：字符串数组，事件各选项的文本，用于辅助识别匹配
- `next_event`：字符串，该事件处理完后要跳转到的下一个事件名
- `fallback_choices`：形如 `[[实际选项数, 优先选择], ...]` 的数组，当实际识别到的选项数与 `option_num` 不一致时，按实际选项数匹配并选择对应的选项

### 黑流树海事件策略与选择结果

黑流策略模块支持 `encounter_rules`。规则使用事件标题 `event_name` 匹配事件，`when` 复用策略条件语法。
例如，先行一步对应的事件标题是“未涉足之树”。多个规则同时满足时，先比较 `rank`，数值小的优先；
相同 `rank` 按规则 `id` 的字典序决定顺序。未命中规则时沿用事件默认配置。

选项可以按 `option_text` 中的文字顺序匹配，也可以同时指定 `option_num` 与 `choose`，在实际选项数
与 `option_num` 相等时选择从 1 开始编号的 `choose`。两种表达方式互斥。`allow_fallback` 默认为
`true`，目标不可用或选择失败时从下向上尝试可用选项；设为 `false` 后，目标选择失败就结束本次选择。

事件判断可读取以下内置页面事实，无须在 `facts` 中重复声明：

| 事实                          | 类型          | 含义                                                     |
| ----------------------------- | ------------- | -------------------------------------------------------- |
| `encounter.name`              | `string`      | 当前识别到的事件标题。                                   |
| `encounter.option_count`      | `int`         | 实际识别到的选项数量。                                   |
| `encounter.available_options` | `string_list` | 本次决策时可用的选项文字。                               |
| `encounter.route_known`       | `bool`        | 进入事件时，是否保留了有效的完整单条剩余路线。           |
| `encounter.remaining_battles` | `int`         | 该路线中当前事件之后的战斗数，仅在路线明确时提供给决策。 |
| `encounter.selected_option`   | `int`         | 实际选中的选项序号，从 1 开始。                          |
| `encounter.selected_text`     | `string`      | 实际选中的选项文字。                                     |
| `encounter.used_fallback`     | `bool`        | 实际选择是否使用了兜底。                                 |
| `encounter.rule_id`           | `string`      | 本次选择请求命中的规则；未命中时为空。                   |

`available_options`、`route_known` 与 `remaining_battles` 供事件决策使用。判断无战斗必须同时要求
`route_known` 为 `true`、`remaining_battles` 为 `0`。空路线、多个落点以及后续节点身份不明确时，
均不能认定无战斗。这里读取进入事件时的计划，不约束后续重新规划。

选择成功后，共享事件插件发出 `RoguelikeEventSelected` 回调，包含 `name`、`option_num`、
`choose_option`、`option_text`、`rule_id`、`rule_description` 与 `used_fallback`。界面日志显示实际选中的
文字；命中规则且未兜底时附带规则的 `description` 作为理由，因此不同策略即使选择同一选项也可以
各自说明理由。黑流会话将实际选择写入页面事实，
随后刷新里程碑。`rule_id` 表示原先的选择请求，使用兜底时不能据此认定目标选项已选中；里程碑应检查
实际选择事实。选择结果不代表干员已派出；对于确认选择就必然获得的物品，可由规则的 `on_selected`
明确更新对应事实。

`on_selected` 是整局事实名到值的映射，只允许已声明且类型匹配的 `run` 事实，不允许写入
`milestone.*`。只有实际选中文字属于该规则的 `option_text` 才应用赋值；兜底选中其他文字时不应用。
这些赋值与本次选择记录统一写入后再刷新里程碑，失败时不保留部分更新。二结局“线人”确认选择
“我受雇于梅兰德”后立即将 `sandtable_a` 设为 `true`，节点结束仍沿用奖励与收尾流程。

页面任务结果同样先校验整组事实和页面变更，再统一写入。已完成页面不能再次接收完成结果，避免
同一次收藏品结果重复累计。里程碑变化沿用现有回调，不增加来源记录；错过里程碑的结束判断仍在
地图观测时执行。

黑流事件默认配置支持 `option_tasks`，以实际选中的完整选项文字为键，以已存在的任务名为值。
匹配后经 `JustReturn` 入口转发到对应任务链，目标任务以自己的名称执行，保留其模板配置、任务名回调
和执行次数统计。由该任务链处理后续页面；本次不再执行事件级 `next_event`。没有对应项时继续原有的
奖励和事件收尾流程。所有配置的任务须在选择之前能够解析，缺少任务时不点击选项。

#### 连续单选项事件

黑流事件默认配置中的 `continue_single_option` 默认为 `false`，目前仅为“安眠一隅”启用。
启用后，每一页重新识别选项，仅在恰好一个选项可用时选择它，不按固定文字、数字或倒序兜底选择。
零个或多个可用选项以及选择确认失败都按选择失败处理。

每次选择继续使用确认按钮及 `LoadingText` 等待链。随后处理奖励和事件对话；出现选项时核对事件标题，
标题仍为当前事件才重新识别下一页，明确识别到地图后才结束整个事件。相邻页面可以使用相同的选项文字。
加载、识别失败和用户停止均不能视为事件完成。此字段只支持黑流，且不能与 `next_event` 或 `option_tasks`
同时启用。每次成功选择仍分别回传实际选项，页面完成只在整个事件结束后报告。
...
```json5
{
    ...
    "priority": [                                             //优先度,有序,顺序即为优先级,越在前面越优先购买,
...

        {
            ...
            "promotion": 2,                                   //不为 0 时，队伍中存在待晋升干员才会购买，默认为 0，不作要求
            "promotion_rarity": 6,                            //统计待晋升干员的稀有度上限，只统计稀有度低于该值的干员，可选，默认为 6（即不限制）
