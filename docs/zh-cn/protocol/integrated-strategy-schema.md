...
::: tip
MAA默认肉鸽策略以“适用于绝大多数用户”与“即便在高难度下亦有一定发挥空间”为目标
如果您认为自己的方案在符合此目标的同时优于 maa 的策略，欢迎前往 maa 主仓库提交 pr

在此之前，请您充分阅读文档，参考已有 json 内容进行优化
确保您的修改符合 maa 的目标，并兼容旧有的关卡作业
:::
...
这里因为没有验证干员是否重复（有重复会点两下反而不编入），所以单个干员组中尽量不要有重复干员
但一个强力干员（如维什戴尔）可以出现在多个干员组中，在阵容完备度检测中可能因为存在于多个干员组而一个人满足大于 1 的 threshold
