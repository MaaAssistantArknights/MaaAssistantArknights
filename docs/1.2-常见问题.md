...
- 确认 MAA `设置` - `连接设置` - `adb 路径` 是否已自动填写，若已填写请忽略这步。若未填写：
  ...
  - Mac 安卓模拟器一般安装在 `/Application/` 目录下，右键点击 <模拟器>.app 后选择显示包内容，在目录下查找 adb 可执行文件即可。
...
- 单开情况 / 多开时 首个模拟器
  ...
  针对模拟器单开情况，参考各个模拟器文档和网易游戏高级游戏开发工程师 @赵青青 的[博客](https://www.cnblogs.com/zhaoqingqing/p/15238464.html)，常见安卓模拟器的 adb 端口如下：
    ...
    纯数字的默认端口可以直接使用 `127.0.0.1:<端口>` 来连接，雷电模拟器进行了封装，也可以使用 `emulator-5554` 进行连接。
