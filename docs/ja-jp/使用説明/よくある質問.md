...
3. **最初に** MAAを **終了してから** `gui.json` を開きます、 `Configurations` の下の現在の構成名フィールドを見つけます（設定 - 構成を切り替える で表示でき、デフォルトは `Default` です）、その中のフィールド `Bluestacks.Config.Path` を検索しますし、 `bluestacks.conf` へのフルパスを記入します。（注意として `\\`をエスケープに使用しています）<br>
