...
- 応接室は各ローテーション時に必ず手がかりの送付を試みます。公式（Official、およびBilibili）クライアントは2025-12-05に「重複手がかりをワンクリックで送る」機能を追加しました。該当ボタンを検出した場合、MAAは自動でクリックして重複手がかりをまとめて送信します。EN/JP/KRは約6か月後、TWは約1年後に対応予定です（台湾版は開発進捗により早まる可能性があります）。
