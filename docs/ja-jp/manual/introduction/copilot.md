...
- `マルチジョブモード` 機能を使用して、同一エリアのステージの自動連続戦闘を行えます。
  - ジョブリスト下部の3つのボタンは左から右へ順に `バッチインポート`、`追加`、`クリア` です。  
    `追加` を左クリックすると通常難易度のステージを追加し、右クリックすると強襲難易度のステージを追加します。`クリア` を左クリックするとすべてのステージをクリアし、右クリックするとチェックされていないステージをクリアします。
  - 作業をインポートすると、ジョブリスト下部にステージ名が表示されます。正しいことを確認してからそのステージを追加してください。リスト内のステージはドラッグで順序を調整でき、実行するかどうかをチェックできます。
