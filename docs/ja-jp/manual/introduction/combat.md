...
- `理性剤使用数` と `割る源石の数`、および `周回数指定`・`素材の指定` はいずれもショートサーキット的なスイッチ（OR条件）です。これら3つの条件のうちいずれかを満たした時点でタスク達成とみなし、作戦を停止します。
  ...
  - `素材の指定` は特定素材を何個入手したら停止するかを指定します（例：「中級源岩を5個入手したら停止」）。2 つのカウントモードをサポートします：
    ...
    - `目標在庫`：[倉庫アイテム認識](./tools.md#倉庫アイテム認識) に保存された在庫データを参照し、設定在庫まで補充します。[データ更新](./user-data-update.md) または [倉庫アイテム認識](./tools.md#倉庫アイテム認識) で在庫データを取得する必要があります。このモードは UI により実装されています（MaaCore はドロップ数モードのみサポート、UI が在庫キャッシュから不足分を計算してドロップ数に変換します）。
