...
::: info UI 専用機能
本ページの一部機能は UI 層で実装されています（目標在庫、高度なスケジュール、複数タスクの並び替えなど）。詳細は[始めに](../newbie.md#このドキュメントについて)を参照してください。
:::
...
- 先に殲滅を回したい場合は、殲滅のみを指定した作戦タスクを別に追加し、既存の作戦タスクより上へドラッグしてください。高度な設定で高度なスケジュールとその下の週次スケジュールを有効にし、月曜日だけにチェックを入れると、月曜日のみ実行されます。
