...
- `目標在庫` モードは[在庫維持](./depot-maintain.md)と開始時判定を共有します：タスク開始時に最新の在庫で不足分を再計算し、達成済みならそのままスキップします。また、このキューの実行中に既に戦闘が報告した理性がステージ消費未満で、かつこのタスクに薬剤/源石の予算がない（臨期薬を使い切ったことが確定している）場合も同様にスキップし、いずれの場合も出撃しません。
