...
- 詳細設定の `殲滅作戦では全権委任のみを使用` はデフォルトで無効です。無効の場合は従来の動作を維持し、全権委任を優先して試行し、利用できなくても通常の自動指揮が利用可能なら続行します。どちらも利用できない場合はタスクが失敗します。
- `殲滅作戦では全権委任のみを使用` を有効にすると、殲滅作戦を開始するたびに全権委任が利用可能かを確認します。代理カードを使い切った場合、または全権委任を利用できない場合、現在の理性消費タスクを正常終了して次のタスクを続行し、通常の自動指揮には切り替えません。
