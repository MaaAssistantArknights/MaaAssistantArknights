...
::: info UI 専用機能
本ページの一部機能は UI 層で実装されています（目標在庫、イベント期間外の実行を優先、週次スケジュール、複数タスクの並び替えなど）。詳細は[始めに](../newbie.md#このドキュメントについて)を参照してください。
:::
...
- 殲滅作戦での使用を推奨します。有効にすると、Side Story イベントが開催中かを確認します。開催中のイベントが今週中に終了する場合は今回の実行をスキップし、それ以外の場合はスキップしません。
