...
| 黑流树海 | _未定_ <!-- 保密等級·0~15 --> | 特別行動分隊 / 要塞戦術分隊 | 攻守一体 / 難攻不落 | 機械師（昇進済み） |
...
| 黑流树海 | 自動戦闘などの専用戦略はまだ作成中のため、関連する判断は汎用戦略に委ねられます。~~皆さん大好きな AI の登場です。~~<br>現在の戦略では、`機械師` の `募集精通 II` が `効果向上 II` 解放後に昇進する際に得られる `構造性原理` が必要です。この精通は解放難易度が低いため、先に解放しておくことを推奨します。`募集精通 II` の `効果向上 I` のみ解放している場合でも利用可能ですが、効率は大きく低下します。<br>上記の `効果向上` を解放していない場合は、**召喚物を持たない**他の強力なオペレーターで開局し、**源石錐収集戦略のみの使用を推奨**します。<br>レベル稼ぎ戦略使用時、`源石錐を投資する` をチェックしない場合、商店はスキップされます。 |
