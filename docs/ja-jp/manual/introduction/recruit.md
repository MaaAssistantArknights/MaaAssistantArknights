...
`優先タグ` を有効にすると、優先したい 3★タグを選択できます。`指定タグを保留` を有効にすると、保持したいタグを選択できます。

保留対象のタグを1つでも認識した場合、MAA はその公開求人枠を保持したまま今回の募集をスキップし、確認操作は行いません。

`星3用に求人票を残す` を有効にして数値を設定すると、求人票の残数がその値以下の場合に MAA は現在の星3募集をスキップします。残数の認識に失敗した場合も同様にスキップし、保留分の求人票を消費しないようにします。
