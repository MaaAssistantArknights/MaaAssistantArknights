...
- 自動更新後に機能が欠落している、または使用できない場合は、更新プロセス中に問題が発生した可能性があります。更新に失敗した場合、MAA は次回起動時に更新失敗の通知を表示し、自動修復（完全パッケージを再ダウンロードして再起動後にインストール）を提案します。その通知を無視した場合はインストールが中途半端な状態になっている可能性があるため、完全なインストールパッケージを再ダウンロードして解凍してください。解凍後、古い `MAA` フォルダから `config`、`data`、`debug` フォルダを新しく解凍した `MAA` フォルダにコピーして、既存の設定とログを保持してください。`cache`（キャッシュ）と `achievement`（クリアスクリーンショット）フォルダは必要に応じて保持できます。
...
### 起動時に「インストールファイルの欠落」と表示される

MAA は起動時に、インストールパッケージに同梱のファイルリスト（filelist.txt）と照合してインストールファイルの欠落を確認します。このプロンプトが表示された場合、一部のファイルがセキュリティソフトによってブロックまたは削除されたか、手動で削除されており、対応する機能（ミニゲームなどのタスク）が動作しない可能性があります：

- セキュリティソフトが原因の場合は、その隔離領域から削除されたファイルを復元し、MAA のインストールフォルダーを許可リストに追加してください。
- ダイアログで自動修復（完全パッケージを再ダウンロードし、再起動後に復元）を選択することもできますし、ダイアログ内の説明に従って手動で再インストールすることもできます。

### DLL インジェクションの問題

MAA の起動時に互換性警告が表示されたり、クラッシュやインターフェースのレンダリング異常が発生する場合、通常は Nahimic などのサードパーティ製ソフトウェアが MAA に DLL を注入していることが原因です。以下のいずれかの方法で解決できます：

**方法 1：Windows セキュリティで DLL インジェクションをブロックする（推奨）**

1. **Windows セキュリティ** → **アプリとブラウザーのコントロール** → **Exploit Protection** → **Exploit Protection 設定** → **プログラム設定** を開く
2. **カスタマイズするプログラムの追加** をクリックし、**「プログラム名で追加」** を選択して `MAA.exe` と入力（または **「正確なファイル パスの選択」** を選択して MAA インストールディレクトリ内の `MAA.exe` を指定）
3. MAA の設定一覧で **「拡張ポイントの無効化」** を見つけ、**「システム設定の上書き」** にチェックを入れてオンにする
4. **「適用」** をクリックして保存し、MAA を再起動する

この設定は、システムレベルでサードパーティ製ソフトウェアによる MAA への DLL 注入をブロックし、他のプログラムには影響しません。

**方法 2：注入された DLL の名前を変更する**

1. 警告に表示されたパスで該当する DLL ファイルを見つける（ファイル拡張子やファイル自体が表示されない場合は、エクスプローラー上部の **表示 → 表示** で **「ファイル名拡張子」** と **「隠しファイル」** を有効にする）
2. ファイルを右クリック → **名前の変更** し、拡張子 `.dll` を `.dll1` などの無効な拡張子に変更する

::: warning
この方法は関連ソフトウェアの動作に問題を起こす可能性があります。慎重に使用してください。原因の詳細は[関連ポスト](https://t.bilibili.com/1133690423484612615)と [blog](https://blog.walterlv.com/post/wpf-renders-wrong-because-of-nahimicosd.html) を参照してください。
:::

**方法 3：関連ソフトウェアの処理**

関連ソフトウェアの除外リストに MAA を追加するか、関連ソフトウェアをアンインストールした後に問題が解決されたか確認してください。
...
- CN以外のクライアントをお使いの場合は、`設定` - `実行設定` - `クライアントバージョン` の選択でご確認ください。また、非CNクライアントでは全ての機能がサポートされているわけではありませんので、各国語のドキュメントを参照してください。
...
2. `完了後` の `エミュレータを終了する`：ターゲットエミュレーターが管理者権限で実行されている場合、管理者権限が必要です。
