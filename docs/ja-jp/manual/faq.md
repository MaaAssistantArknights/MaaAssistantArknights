...
- 自動更新後に機能が欠落している、または使用できない場合は、更新プロセス中に問題が発生した可能性があります。完全なインストールパッケージを再ダウンロードして解凍してください。解凍後、古い `MAA` フォルダから `config`、`data`、`debug` フォルダを新しく解凍した `MAA` フォルダにコピーして、既存の設定とログを保持してください。`cache`（キャッシュ）と `achievement`（クリアスクリーンショット）フォルダは必要に応じて保持できます。
