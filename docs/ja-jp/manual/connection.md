...
MAA は現在**実行中の単一のエミュレータ**を自動検出し、ADB パス・接続アドレス・接続設定を自動的に入力できます。

MAA v5.22.3 時点で自動検出に対応しているエミュレータと接続アドレスは以下のとおりです。

- BlueStacks 5：`127.0.0.1:5555/5556/5565/5575/5585/5595/5554`
- MuMu エミュレータ：`127.0.0.1:16384/16416/16448/16480/16512/16544/16576`
- LDPlayer 9：`emulator-5554/5556/5558/5560`、`127.0.0.1:5555/5557/5559/5561`
- 夜神エミュレータ：`127.0.0.1:62001/59865`
- 逍遥エミュレータ：`127.0.0.1:21503`
- テンセントアプリストア (5.10.56.xx 以降)：`127.0.0.1:5555`

検出に失敗した場合は、UAC 管理者権限で MAA を起動して再度検出してください。

それでも失敗する場合は、下記の手動設定を参照してください。また、お使いのエミュレータと接続アドレスが上記のリストに含まれているかも確認してください。
...
- 方法2：すでに確立されたADB接続を検索する
  ...
  3. `ネットワーク` タブに切り替えて、モニターするポート名であるエミュレータプロセス名（例：`HD-Player.exe`）を検索します。
  4. エミュレータプロセスのすべてのリッスンポートを記録します。
  5. `TCP接続` タブの中で、`adb.exe` を探し、リモートポート列でエミュレータリッスンポートに一致するポートを探します。

:::

### BlueStacksエミュレータ Hyper-V 启動ごとにポート番号が異なります

`接続設定` で `接続構成` を `BlueStacksエミュレータ` に設定し、その後 `自動接続の検出` と `毎回再検出` をチェックします。

通常、これで接続できるはずです。接続できない場合、複数のエミュレータコアが存在するか、問題が発生している可能性がありますので、以下の追加設定をお読みください。

#### `ConfigKeyword` を指定

::: info 注意
複数の起動機能が有効になっている場合や複数のエミュレータコアがインストールされている場合は、追加の設定が必要です。
:::

`gui.new.json` 内で現在の設定名フィールド（設定-切り替え設定 で確認できます。デフォルトは `Default`）の下にある `Gui.ConnectSettings.Extras.BluestacksExtra.ConfigKeyword` フィールドを検索し、内容を `"bst.instance.<エミュレータ番号>.status.adb_port"` に設定します。エミュレータ番号はエミュレータパスの `BlueStacks_nxt\Engine` 内で確認できます。
...
1. ブルースタックスエミュレータのデータディレクトリ内にある `bluestacks.conf` ファイルを見つけます。
...
対応するエミュレータの設定を選択してください。リストにない場合は汎用設定を選択してください。汎用設定が利用できない場合は他の利用可能な設定を試し、選択してください。

具体的な違いについては、[ソースコード](https://github.com/MaaAssistantArknights/MaaAssistantArknights/blob/dev-v2/resource/config.json#L57)をご覧ください。
...
2. `MuMu12 エミュレータパス` には `MuMu Player` または `MuMuPlayerGlobal-12.0` または `YXArkNights-12.0` フォルダのパスを入力してください。例: `C:\Program Files\Netease\MuMuPlayerGlobal-12.0`。
...
3. `インスタンス番号` には LD マルチインスタンス内でのエミュレータの番号を入力してください。主インスタンスの場合は `0` です。

### AVD スクリーンショット強化モード

Android Emulator v27.2.9 以降が必要です。（アークナイツのリリース以降にダウンロードしたエミュレータであれば大丈夫です。）
...
:::info 仕組み
複数のエミュレータを同時に操作する場合、MAAフォルダを複数コピーし、**異なるMAA**、**同じadb.exe**、**異なる接続アドレス** を使用して接続します。
:::

### 自動的に複数のエミュレータを起動する

**[ブルースタックス国際版](./device/windows.md)を例に**、2つの多重起動エミュレータの開始方法を紹介します。

#### エミュレータexeにコマンドを添付して複数起動する

1. **単一**のエミュレータを複数開始します。
2. タスクマネージャーを開き、該当するエミュレータプロセスを見つけます。詳細情報タブに移動し、列ヘッダーで右クリックし、「列の選択」をクリックし、「コマンドライン」を選択します。
...
```text
多開1:
エミュレータパス: C:\Program Files\BlueStacks_nxt\HD-Player.exe
...
#### エミュレータやアプリを使用した複数起動の方法

1. マルチインスタンス管理ツールを開き、対応するエミュレータのショートカットを追加します。
2. エミュレータのショートカットのパスを「起動設定」-「エミュレータのパス」に入力します。

::: tip
一部のエミュレータはアプリケーションのショートカットを作成することができ、それを使用してエミュレータを直接起動し、明日方舟を開くことができます。
:::
...
```text
多開1:
エミュレータのパス: C:\ProgramData\Microsoft\Windows\Start Menu\Programs\BlueStacks\多開1.lnk
多開2:
エミュレータのパス: C:\ProgramData\Microsoft\Windows\Start Menu\Programs\BlueStacks\多開2-明日方舟.lnk
```

:::

もし `エミュレータのパス` を使用して複数の起動操作を行う場合、`起動設定` - `追加コマンド` を空に設定することをお勧めします。
