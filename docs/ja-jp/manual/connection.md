...
公式版 MuMu 12 V4.0.0 またはそれ以降のバージョンを使用し。または国際版の MuMu 12 V4.1.26 以降のバージョンを使用する必要があります。<!-- V3.8.13 -->

1. `設定` - `接続設定` で `MuMu のスクリーンショット拡張モードを有効にする` をチェックします。MAAは、スイッチをチェックすると、レジストリを通じてインストールパスを自動的に入力しようとします。

2. `MuMu12 エミュレーターパス` には `MuMuPlayer-12.0` または `MuMuPlayerGlobal-12.0` フォルダのパスを入力してください。例: `C:\Program Files\Netease\MuMuPlayer-12.0`。
