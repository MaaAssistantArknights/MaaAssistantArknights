...
MAA は、固定間隔でこのエンドポイントを継続的にポーリングし（デフォルトは 1 秒、設定で変更可能）、実行するタスクを取得しようとし、取得したリストの順序で実行します。
...
```json
{
    ...
    [
        ...
        {
            "id": "b353c469-b902-4357-bd8f-d133199eea31",   //タスクの一意の ID である文字列型、タスクがデブリーフィングされるのときに使用されます
            "type": "Settings-ConnectAddress",              //構成項目を変更するタスク、接続設定の ConnectAddress プロパティを設定することと同じです; セキュリティ上の理由から、すべての構成を変更できるわけではなく、変更できる構成を以下に詳しく説明します。
            "params": "value"                               //要修改的值
        },
    ...
    "progressReport": true,             // 任意フィールド。このサーバーが「タスクごとの進捗報告」に対応していることを宣言します（詳細は後述の「タスクデータの報告エンドポイント」の拡張セクションを参照）。省略または false の場合、MAA は旧プロトコルどおり最終報告のみを送信し、未宣言時とまったく同じ動作になります。
    ...     // このエンドポイントに他の用途がある場合は、オプションの戻り値を自分で追加できますが、MAA は "tasks" と "progressReport" のみを読み取ります
}
```
...
エンドポイントの戻り値は任意です。MAA は戻り値を読み取らず、ステータスコードも検証しません。報告リクエストが失敗した場合、MAA はログにエラーを記録するだけです

### タスクごとの進捗報告（拡張）

::: warning
本セクションで説明する中間報告は、サーバーがタスク取得エンドポイントの応答で `"progressReport": true` を宣言した場合にのみ送信されます。このフィールドはポーリングごとに更新されるため、サーバーはいつでも切り替えられます。宣言していないサーバーは中間報告を一切受け取らず、動作は旧プロトコルと完全に同一です。
:::

リモート制御で `LinkStart` シリーズのタスクを送信した場合、MAA は最終報告とは別に、実行中の中間報告も送信し、タスクリストの各項目の開始／終了をリアルタイムに通知します。中間報告は最終報告と同じメッセージ構造を使用し、以下の点が異なります：

- `status` は `"RUNNING"` で中間状態を表します。実行終了後には `SUCCESS`／`FAILED` の最終報告が改めて送信され、これが締めとなります。
- `payload` は UTF-8 エンコードされた JSON テキストで、以下のフィールドを含みます：
  - `seq`：単調増加するメッセージ通番。HTTP リクエストは到達順序を保証しないため、サーバーは `seq` により時系列を復元し、冪等に処理してください。
  - `event`：イベント種別。値は以下のとおりです
    - `QUEUED`：タスクを受け付け、まもなく実行を開始します。`plan` 配列（タスクリストのスナップショット）を伴い、各項目は `index` / `name` / `type` / `result`（`PENDING` または `SKIPPED`）を含みます。
    - `TASK_START`：タスクリストのいずれかの項目が実行を開始しました。`index` / `name` / `type` を伴います。
    - `TASK_END`：タスクリストのいずれかの項目が実行を終了しました。`index` / `name` / `type` / `result`（`SUCCESS` または `FAILED`）を伴います。戦闘系タスクでは `stages` 配列が付く場合があり、ステージごとの戦闘回数とドロップ統計を示します：
      - `stage`（string）：ステージコード（例：`1-7`）。
      - `times`（number）：そのステージで完了した戦闘回数。
      - `drops` 配列：そのステージで獲得したドロップ。各項目は `id`（アイテム ID）/ `name`（アイテム名）/ `count`（そのステージの累計数）を含みます。`id` は通常ゲーム内のアイテム ID と一致しますが、ラッキードロップは例外です（後述の注記を参照）。
      - 1 つのタスク項目が複数のステージを周回する場合（例：在庫維持の複数プラン）、`stages` はステージごとに列挙されます。同一ステージを複数プランで周回する場合、`times` と `count` は加算されます。
    - 戦闘系タスクでは `sanity` オブジェクトが付く場合があります：ステージ突入前に認識した理性のスナップショット（今回の実行で最後にステージへ入る前の値）で、`current` / `max`（現在の理性と上限）に加え、`recoverFullAt`（タイムゾーン付き ISO 8601 の全回復予測時刻）と `recoverMinutes`（送信時に算出した残り分数）を含みます。推定は自然回復 6 分で 1 ポイントの基準です。認識に失敗した場合や今回ステージに入らなかった場合は省略されます。
    - 自動公開採用タスクでは `minLevel` 配列（number[]）が付く場合があります：今回の実行で採用を開始した各枠について、実際に有効だった保証星級を実行順に並べたものです。タグを選択した場合はその組み合わせに一致するオペレーターの最低星級（1〜6、MAA が採用すべきか判断する際の基準と同一）、タグを選択せずに採用した場合はタグなしの基準として `3` を記録します。スキップまたは更新した枠は含まれません。
    - `ALL_COMPLETED`：今回の実行の締め。`summary` 配列（構造は `plan` と同一で、各項目の `stages` を含む場合があります）を伴い、`result` は `SUCCESS` / `FAILED` / `SKIPPED` / `STOPPED` のいずれかです。最終報告が正とされ、`ALL_COMPLETED` は簡易表示用です。

::: note

- 中間報告を生成するのは `LinkStart` シリーズのタスクのみです。スクリーンショットやハートビートなどのタスクは生成しません。
- ラッキードロップには対応するアイテムテンプレート画像がないため、MAA は元のアイテムを識別できません。表示上の慣例に合わせて家具パーツ（`id` は `3401`）に正規化し、UI 言語に対応する名称（例：`家具` / `Furniture`）を付けます。この項目の `id` はゲーム内の実際のアイテム ID とは異なります。
- タスク取得応答で `progressReport` を宣言していないサーバーは、中間報告を一切受け取りません（新旧どちらの MAA でも同様です）。
- `progressReport` を宣言したサーバーは `"RUNNING"` を中間状態として扱い、`SUCCESS`／`FAILED` の最終報告をタスクの締めとしてください。
