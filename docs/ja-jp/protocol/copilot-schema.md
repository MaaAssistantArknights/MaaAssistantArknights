    ...
    "actions": [                    // 順番に行動、必須. 順次実行されます
        {
            "type": "Deploy",       // 行動タイプ、オプション、デフォルトは "Deploy"
                                    ...
                                    // "Click" "rect" で指定したピクセル領域、または "location" で指定した戦場マスを認識なしで直接クリックします。空きマスもクリック可能です
                                    //      "rect" と "location" はどちらか一方のみ指定できます。両方指定、または両方未指定の場合はエラーになり、このステップはスキップされます
                                    // "Swipe" "begin" 矩形から "end" 矩形へスワイプします。下位レベルのスワイプ命令を直接送信します
            ...
            "location": [ 5, 5 ],   // 配置位置
                                    ...
                                    // type が "Click" の場合は "rect" とどちらか一方のみ指定。このマスを直接クリックします
            "rect": [ 100, 100, 50, 50 ],
