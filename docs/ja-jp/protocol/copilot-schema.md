    ...
    "actions": [                    // 順番に行動、必須. 順次実行されます
        {
            "type": "Deploy",       // 行動タイプ、オプション、デフォルトは "Deploy"
                                    ...
                                    //      "rect" と "location" はどちらか一方のみ指定できます。両方指定、または両方未指定の場合は解析エラーとなり、ジョブの読み込みに失敗します
            ...
            "location": [ 5, 5 ],   // 配置位置
                                    // `type` が "Deploy" の場合必須.
                                    // type が "Click" の場合は "rect" とどちらか一方のみ指定。このマスを直接クリックします。座標は [0, 0] を含む任意の有効なマスを指定できます
            ...
            "rect": [ 100, 100, 50, 50 ],
                                    // type が "Click" の場合は "location" とどちらか一方のみ指定。720p 基準のピクセル矩形 [x, y, w, h] です
