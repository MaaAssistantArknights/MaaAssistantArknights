...
```json
{
    "TaskName" : {                          // タスク名
        ...
        "action": "ClickSelf",              // オプション、認識後のアクションを示します
                                            ...
                                            //     - SwipeSelf: 認識した位置からスライドします、 rectMove フィールドに対応します
        ...
        "rectMove": [ 0, 0, 0, 0 ],         // オプション、認識後のターゲット移動、このオプションは推奨されません。 1280 * 720 を基準とした自動スケーリング
                                            ...
                                            // 追加的に、 action が Swipe または SwipeSelf の場合、スライドの終了を示します。
        ...
        "specialParams": [ int, ... ],      // 一部の特殊な認識器に必要なパラメーター
                                            // 追加、 action が Swipe または SwipeSelf の場合はオプション、 [0] が duration 、 [1] が追加のスライドを有効にするかどうか
