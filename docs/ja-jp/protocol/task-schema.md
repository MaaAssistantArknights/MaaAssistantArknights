...
```json
{
    "TaskName" : {                          // タスク名
        ...
        "cache": false,                      // オプション、タスクがキャッシュを使用するかどうかを示します、デフォルトは false です
                                            // 有効にすると、最初にターゲットを認識した後、常に最初に認識された位置のみで認識を行い、パフォーマンスを大幅に節約できます
                                            // ただし、特定する対象場所が全く変わらないタスクのみ適用され、特定する対象場所が変わる場合はデフォルトの false を維持してください
        ...
        "specialParams": [int, ... ],      // 一部の特殊な認識器に必要なパラメーター
                                            // 追加、 action が Swipe の場合はオプション、 [0] が duration 、 [1] が追加スライドの方向（0 は無効、1/2/3/4 はそれぞれ上/下/左/右）、
                                            // [2] と [3] はそれぞれスライド軌跡の緩入・緩出の傾斜を示し、10 倍して入力する必要があり、デフォルトはどちらも 10
                                            // 通常の緩入・緩出にしたい場合、[2] と [3] はそれぞれ 37 と 1 を推奨
        ...
        "nmsDistance": 0,                   // オプション、テンプレートマッチング複数結果の重複排除半径、単位はピクセル、デフォルトは 0
                                            // 2 つの命中位置の横縦座標差がいずれもこの値未満の場合、スコアが最も高いもののみ残す
                                            // 未記入または 0 の場合はテンプレート短辺の半分を使用
        ...
        "replaceFull": false,               // オプション、ocrReplace が一致時に認識テキスト全体を置換するかどうか。デフォルトは false
                                            // false の場合は一致した部分文字列のみ置換、true の場合はテキスト全体がいずれかの置換ルールに一致した場合、テキスト全体がそのルールの置換値で置き換えられます

        "isAscii": false,                   // オプション、認識されるテキストコンテンツが ASCII 文字であるかどうか
                                            // 指定されていない場合、デフォルトはfalse

        "orderBy": "None",                  // オプション、複数マッチ時の結果の並び順（省略時は baseTask を継承、base がなければ並べ替えなし・認識順）
                                            //      - None:         並べ替えなし（認識順）。base で設定された並び順を上書きするには明示的に指定
                                            //      - Horizontal:   行優先（行内で左から右）
                                            //      - Vertical:     列優先（列内で上から下）
                                            //      - Score:        スコアの高い順
