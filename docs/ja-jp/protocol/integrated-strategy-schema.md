...
- `JieGarden/` の下の
  ...
  - `map.json` は界園ローグのマップノードのテンプレート画像情報を保存
           ...
           "opers": [                                      // これにはどのオペレーターが含まれ、順序があり、配置の優先度を表します
               {
                   ...
                   "role": "Warrior",                       // オペレーターの職業、オプション。同名オペレーターが存在する場合に職業を区別するために使用。英語の職業名を記入、大文字小文字は不問
                   ...
                   "is_alternate": false,                  // true はバックアップオペレーター（重複募集を許可、バックアップオペレーターにスワイプされた場合はそれ以上右へスワイプしない）、false または省略で通常オペレーター
                   ...
                   "collection_priority_offsets": [         // 現在所持しているコレクションに応じた募集優先度の調整
                       {
                           "collection": "古旧的钱币",       // コレクション名
                           "offset": 100                    // そのコレクションを所持した後の募集優先度の調整、省略でデフォルト 0
                       }
                   ]
               },
               ...
...
{
    "theme": "Sami",                              //ローグテーマ
    "mode": [1, 4],                               //このファイルがどのモードで有効になるかを限定します、オプション。省略時はすべてのモードで有効
    "stage": [                                    //思わぬ遭遇イベント
        {
            ...
            "choices": [                          //特定のオプションに対する追加要件
                {
                    "name": "选择碎草药",          //オプションの名
                    "choose": 1,                  //下記の条件を満たす場合に優先的に選択するオプションの番号、オプション。省略時はこの項目は無効
                    "requirements": [             //発効条件、オプション
                        {
                            "name": "Vision",     //条件タイプ、現在は Vision（灯火値）のみサポート
                            "value": "3",         //必要な数値
                            "type": ">"           //より大きいか小さいか（= も可。ここでは、灯火が3より大きいときにこの項目の choose が有効になることを意味します）
                        }
                    ]
                },
                {
...
イベントではさらに以下のオプションフィールドをサポートしています：

- `option_text`：文字列の配列。イベントの各選択肢のテキスト。認識マッチングの補助に使用
- `next_event`：文字列。このイベントの処理完了後にジャンプする次のイベント名
- `fallback_choices`：`[[実際の選択肢数, 優先選択], ...]` 形式の配列。実際に認識された選択肢数が `option_num` と一致しない場合、実際の選択肢数に応じて対応する選択肢を選択
...
```json
{
    ...
    "priority": [                                           //優先度、有序、順序はすべて優先度を表し、前にあるほど優先的に購入されます
...

        {
            ...
            "promotion": 2,                                 //0 でない場合、昇進待ちのオペレーターがチームにいるときにのみ購入します。デフォルトは 0 で、要件なし
            "promotion_rarity": 6,                          //昇進待ちオペレーターを統計するレアリティの上限。この値より低いレアリティのオペレーターのみ統計します。オプション、デフォルトは 6（制限なし）
