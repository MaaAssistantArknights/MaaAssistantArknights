...
```json
{
    ...
    "groups": [
        // オペレーター編成、オプション。copilot プロトコルの groups と同じ使い方。actions の name には編成名を記入できます
        {
            "name": "地面阻挡",
            "opers": [
                {
    ...
    "tool_men": {                          // 各職業の必要な残りの人員数、コストでソートされています、必須
                                           // 現在のバージョンでは実装されておらず、インタフェース上の表示のみです
    ...
    "stages": [
        {
            ...
            "draw_as_possible": true,   // 「支援要請」ボタン、利用可能な場合に使用するかどうか。必須です。
            "actions": [
                // オプション
            ...
            "retry_times": 3 // バトルが失敗した場合の再試行回数。オプション、デフォルトは 0。この制限を超えると、ステージ全体を諦めます。
        },
        {
            "stage_name": "见者有份"
            // ...
