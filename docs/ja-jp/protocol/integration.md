...
- 復刻 SS イベント。`SSReopen-<ステージ プレフィックス>` と入力します。例えば `SSReopen-IC` と入力すると、XX-1～XX-9 ステージを一度に完了できます。
  ...
  <br>
  ::: info サーバー差異
  Core はゲーム内の連戦リストのスタイルから OCR で利用可能な回数を動的に識別し、上限をハードコードしていません。入力の検証はリソースバージョンに依存します：
  <br>
  中国版サーバーは 2026/8/1 のアップデート後に新リストスタイルを採用し、`-1～10` を受け付けます。海外サーバーはまだ旧スタイルで、`-1～6` のみ受け付け、それより大きい値は拒否されます。海外サーバーは約半年後に追従予定で、その際に上限が自動的に有効になります。
  :::  
  :::  
