...
4. To debug MaaFwAdbController (MaaFwAdb touch mode) / MaaFwLinuxController (MaaFwLinux touch mode) features, you need to [compile the Debug version of MaaFramework yourself](https://maafw.com/docs/4.1-BuildGuide) and put `libMaaAdbControlUnit.so` / `libMaaLinuxControlUnit.so` in the installation directory.
