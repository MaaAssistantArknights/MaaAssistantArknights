...
2. MAA will no longer be falsely reported by Windows Defender. ~~For this plate of vinegar-wrapped dumplings~~
3. MAA 5.0 version (WpfGui) supports at least Windows 10 1809 or above. For users whose system is lower than this version, it is recommended to use [4.28.8](https://github.com/MaaAssistantArknights/MaaAssistantArknights/releases/tag/v4.28.8) version
4. [.NET 8 does not support Windows 7/8/8.1 systems](https://github.com/dotnet/core/issues/7556), so MAA is also no longer supported, even though it can still run now.
~~5. When running Windows 7 version 5.0.0 or above, abnormal memory usage may occur. Please refer to the Windows 7 section below for mitigation measures. Windows 8/8.1 has not been tested. If you have the same problem, please send an Issue to remind us to supplement the documentation. ~~
