...
- Automatically detects morale levels; during shift changes, stationed operators with remaining morale percentage below the `Base Facility Morale Threshold` are swapped out and then assigned to dormitories in the dormitory filling phase.
...
- Fiammetta recovery targets: this feature is disabled by default and takes effect after enabling `Enable Fiammetta morale recovery` in the base settings. Once enabled, up to 3 recovery targets (`Purestream`, `Closure`, `Proviso`, `Shamare`, `Tequila`, `Gladiia`) can be selected. At the start of a shift change, the target operator with the lowest morale is placed in a dormitory together with Fiammetta to swap moods, after which only Fiammetta stays in the dormitory to recover. Since the 007 schedule of `Shamare` and `Tequila` is difficult to achieve with automatic shift changes, selecting these two operators is not recommended.
...
- Enabling `Do not place stationed operators in dormitory` prevents operators like `Irene` and `Logos` from being assigned to dormitories when not training in the Training Room. Once enabled, stationed operators are no longer pulled into dormitories at the start of a shift; operators with low morale are instead moved to dormitories after being swapped out during their facility's shift change.
