...
- Enabling `Do not place stationed operators in dormitory` prevents operators like `Irene` and `Logos` from being assigned to dormitories when not training in the Training Room; stationed operators with low morale are moved to dormitories after being swapped out during their facility's shift change.
