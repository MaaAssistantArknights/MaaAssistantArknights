...
| 黑流树海 | _None yet_ <!-- 保密等级·0~15 --> | Special Squad / Tactical Fortification Squad | Slow and Steady Wins the Race / Indestructible | `Mechanic` (Elite 2 and `Effect Improvement II` of `Recruitment Mastery II` required) |
...
| 黑流树海 | Dedicated combat strategies are still being written; decisions are currently made by the generic strategy. ~~Here comes the AI you all love.~~<br>The current strategy requires the `Mechanic` at Elite 2 with the `Effect Improvement II` of the `Recruitment Mastery II` talent unlocked, so that a `Structural Principle` part (fly anywhere on the map 3 times) is carried from the start; the talent is easy to unlock (just sell 10 parts in total after Elite 2) and recommended to prioritize. Once the Mechanic is recruited, the promotion check is skipped and the strategy runs as if it were Elite 2 for efficiency; a non-Elite 2 Mechanic will get stuck or loop forever. Some events also have different options before the first clear and may get stuck; clearing N1 once satisfies both the Elite 2 requirement and the event options.<br>If these requirements are not met, you must start with another strong operator **without summons**, and **only the Originium Ingot farming strategy is recommended** (battles will most likely fail; other strategies await future optimization).<br>When using the level farming strategy with `Invest ingots` unchecked, shops will be skipped. |
