...
1. Install Visual C++ Redistributable
   ...
   ::: tip
   `DependencySetup_依赖库安装.bat` relies on winget, which typically doesn't work properly in Wine, so manual installation of runtime libraries is required.
   :::
...
1. Install MAA Dynamic Library
   ...
   Choose one of the following installation methods:
   - Download the precompiled Linux dynamic library tarball from the [MAA website](https://maa.plus/), extract it, and edit the `Python/sample.py` file
   - AUR: [maa-assistant-arknights](https://aur.archlinux.org/packages/maa-assistant-arknights), follow the post-installation instructions under "Alternative usage" and edit the file
   ...
   ::: tip
   The precompiled dynamic library is cross-compiled based on the [MaaLinuxToolchain](https://github.com/MaaXYZ/MaaLinuxToolchain) toolchain and requires glibc version as low as 2.31 (Ubuntu 20.04). If you still encounter ABI incompatibility issues (such as on non-glibc distributions), you can run it inside a container or refer to the [Linux Compilation Tutorial](../../develop/linux-tutorial.md) to recompile.
   :::

2. ADB Configuration
   1. In the `sample.py` file opened in the previous step, find the line [`if asst.connect("adb.exe", "127.0.0.1:5555"):`](https://github.com/MaaAssistantArknights/MaaAssistantArknights/blob/dev-v2/src/Python/sample.py#L71).
   ...
   4. Now you can test with `$ python3 sample.py`. If it outputs "连接成功", you're ready to proceed.

3. Task Configuration

   Read [Integration Documentation - Task Types Overview](../../protocol/integration.md#task-types-overview) and modify the `asst.append_task(…)` function calls in `sample.py` according to your needs.
...
The game only supports the ARM architecture. On x64, you need to install an arm64 translation layer like libhoudini or libndk. See [waydroid_script](https://github.com/casualsnek/waydroid_script) and [Waydroid Helper](https://github.com/waydroid-helper/waydroid-helper).

Waydroid lacks the `/dev/input/eventN` devices necessary for Minitouch, so Minitouch will fail. Please switch to other touch modes.
