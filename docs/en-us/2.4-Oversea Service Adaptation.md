...
`original roi` is the area selected by the mouse. `amplified roi` is the enlarged area, you need the enlarged area, so the `roi` field in `task.json` is filled with this value.
