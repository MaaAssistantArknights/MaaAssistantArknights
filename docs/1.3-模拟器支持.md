...
- 由于模拟器问题，部分对安卓系统要求较高的安卓软件可能不兼容（明日方舟及maa正常运行）
...
- 模拟器在中低端机型易造成卡死（模拟器卡死&画面不动），因此低配机型不推荐使用
- 模拟器不支持卡帧部署（按Exc会延迟2秒暂停，虽然maa无需该功能），因此技术用户不推荐使用
- 运行maa时可能会出现随机关闭明日方舟回到桌面且无任何提示的情况，导致maa仍然执行各种操作，安装一堆广告游戏
...
- 普通MuMu安卓版本为6.0，部分安卓软件可能不兼容（明日方舟及maa正常运行）
...
支持。但测试较少，可能存在未知问题（暂未检出任何问题，欢迎反馈）
