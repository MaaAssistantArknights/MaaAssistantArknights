...
4. 在 Windows 7 运行 MAA 时，会出现内存占用异常的问题，请参阅下文 Windows 7 部分实施缓解措施。Windows 8/8.1 未经测试，若存在相同问题，请顺手发个 Issue 提醒我们补充文档。
:::
...
临时解决 .NET 8 应用在 Windows 7 中的异常内存占用问题 [#8238](https://github.com/MaaAssistantArknights/MaaAssistantArknights/issues/8238)

  1. 打开 `计算机`，右键空白处点击属性，点击左侧 `高级系统设置`，点击 `环境变量`。
...
安卓手机助手会使用 adb 与手机通讯，这类软件有可能会干扰 MAA 运行，请关闭手机助手和现有 adb 进程后重试。
