...
支持。在 MAA 中，点击 `设置` - `连接设置`，`触控模式` 选择 `MacPlayTools` 以外的其他模式，如 `maatouch`。`连接地址` 填入 `127.0.0.1:[端口号]`。MuMu 模拟器启动一个安卓设备后，可以在菜单栏的 `工具` 中查看端口号，该栏下有一个 `打开 ABD (XXXXX)` 括号内的内容即为端口号。创建的第一个设备的端口号通常为 `16384`，在这种情况下连接地址应填入 `127.0.0.1:16384`。
