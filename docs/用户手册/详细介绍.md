...
- 使用 [模拟器内置 adb](./常见问题.md#确认-adb-及连接地址正确) 或自行下载 [adb](https://dl.google.com/android/repository/platform-tools-latest-windows.zip) 并解压。
...
**以[蓝叠国际版](./模拟器和设备支持/Windows模拟器.md#✅-蓝叠模拟器-5-国际版（最稳定👍）)为例**，介绍两种启动多开模拟器的方式。
