...
**接口極速更新中，本文件可能已經過時，建議您參考簡中或英文文件**
...
// 對應的任務參數
{
    ...
    "client_type": string,       // 客戶端版本，可選，預設為空
                                 // 選項："Official" | "Bilibili" | "txwy" | "YoStarEN" | "YoStarJP" | "YoStarKR"
    "start_game_enabled": bool   // 是否自動啟動客戶端，可選，預設不啟動
}
```
...
// 對應的任務參數
{
    ...
    "drops": {                  // 指定掉落數量，可選，預設不指定
        "30011": int,           // key - item_id, value 數量. key 可參考 resource/item_index.json 文件
        "30062": int            // 是或的關系
    },
    ...
    "server": string,           // 服務器，可選，預設 "CN", 會影響掉落辨識及上傳
                                // 選項："CN" | "US" | "JP" | "KR"
    "client_type": string,      // 客戶端版本，可選，預設為空。用於遊戲崩潰時重開並連回去繼續刷，若為空則不啟用該功能
                                // 選項："Official" | "Bilibili" | "txwy" | "YoStarEN" | "YoStarJP" | "YoStarKR"
...
另支援少部分資源關卡名請參考[集成示例](..\tools\AutoLocalization\example\zh-cn.xaml#L260)
...
// 對應的任務參數
{
    ...
    "confirm": [                // 會去點擊確認的 Tag 等級，必選。若僅公招計算，可設定為空數組
        int,
        ...
    ],
    ...
    "server": string,           // 服務器，可選，預設 "CN"，會影響上傳
                                // 選項："CN" | "US" | "JP" | "KR"
}
```
...
{
    "enable": bool,             // 是否啟用本任務，可選，預設為 true
    "filename": string,         // 作業 JSON 的文件路徑，絕對、相對路徑均可。不支援執行中設定
    "formation": bool           // 是否進行 “快捷編隊”，可選，預設否。不支援執行中設定
}
```

作業 JSON 請參考 3.3-戰鬥流程協議.md
...
{
    "enable": bool,
    "task_names": [     // 執行數組中第一個匹配上的任務（及後續 next 等）
...
{
    "enable": bool,
    "filename": string, // 影片的文件路徑，絕對、相對路徑均可。不支援執行中設定
}
```
...
- `const char* params`<br>
    ...
    未標注 “不支援執行中設定” 的字段都支援實時修改；否則若當前任務正在執行，會忽略對應的字段
