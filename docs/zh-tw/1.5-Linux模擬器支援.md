...
2. `adb` 工具調用
   ...
   - 如果模擬器使用 `Android Studio` 的 `avd` ，其自帶 `adb` 。可以直接在 `adb.exe` 一欄填寫 `adb` 路徑，一般在 `$HOME/Android/Sdk/platform-tools/` 裡面可以找到，例如：
...
自定義任務： 根據需要參考 [3.x 集成文檔](https://maa.plus/docs/3.1-%E9%9B%86%E6%88%90%E6%96%87%E6%A1%A3.html) 對 `sample.py` 的 [`# 任務及參數請參考 docs/集成文檔.md`](https://github.com/MaaAssistantArknights/MaaAssistantArknights/blob/722f0ddd4765715199a5dc90ea1bec2940322344/src/Python/sample.py#L54) 一欄進行修改

## 模擬器支援

### ✅ [AVD](https://developer.android.com/studio/run/managing-avds)

必選配置： 16:9 的螢幕分辨率，且分辨率需大於 720p
...
::: tip
以下方案通常對內核模組有一定要求，請根據具體方案和發行版安裝合適的內核模組
:::

### ✅ [Waydroid](https://waydro.id/)

安裝後需要重新設定分辨率（或者大於 720P 且為 16:9 的分辨率，然後重新啟動）：
...
設定 adb 的 IP 地址：打開 `設定` - `關於` - `IP地址` ，記錄第一個 `IP` ，將 `${記錄的IP}:5555` 填入`sample.py` 的 adb IP 一欄。
