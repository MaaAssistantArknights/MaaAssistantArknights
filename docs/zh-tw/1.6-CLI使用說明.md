...
- 通過 TOML、JSON、YAML 格式的配置文件定義 MAA 任務流程，並通過 `maa run <task>` 命令執行任務。
...
對於最新版本的 MAA，CLI 已經打包在 MAA 壓縮檔中，如果你下載了最新版 MAA，解壓縮後就可以使用 CLI，無需額外安裝。 **注意**: 請不要將庫文件和資源文件隨便移動到其他位置，否則 CLI 將無法找到它們。 如果你想將它們移動到其他位置，你可以通過以下命令：
...
如果你目前使用的是舊版本的 MAA 或者希望單獨安裝 CLI，你可以前往 [CLI Release頁面](https://github.com/MaaAssistantArknights/maa-cli/releases/latest) 根據你的架構下載對應的預編譯的 CLI 二進制文件（對於 x86_64 架構，你可以下載以 `x86_64-unknown-linux-gnu.tar.gz` 結尾的文件，對於 aarch64 架構，你可以下載以 `aarch64-unknown-linux-gnu.tar.gz` 結尾的文件），解壓縮後將 `maa` 文件移動到你任何在 `PATH` 環境變量中的目錄下, 然後使用 `maa install` 命令安裝 MAA 共享庫和資源。

### macOS

macOS 用戶和 Linux 用戶一樣，可以在 [CLI Release 頁面](https://github.com/MaaAssistantArknights/maa-cli/releases/latest)下載對應的預編譯的CLI二進制文件（macOS提供通用的二進制文件，以 `universal-apple-darwin.zip` 結尾），解壓縮後將 `maa` 文件移動到你任何在 `PATH` 環境變量中的目錄下，然後使用 `maa install` 命令安裝 MAA 共享庫和資源。
...
你的配置文件（選項、任務等）位於配置目錄中，你可以通過 `maa dir config` 獲取配置目錄，並通過 `mkdir -p "$(maa dir config)"` 建立它。在以下的介紹中，配置目錄的將被稱為 `$MAA_CONFIG_DIR`。此外儘管所有的例子都是針對 TOML 格式的，但是以下所有配置文件的格式可以是 TOML、JSON 或 YAML。
...
[connection]
type = "ADB"
adb_path = "adb" # adb 可執行文件的路徑
device = "emulator-5554" # 你的 android 設備的序列號
config = "General" # maa connect 的配置名稱，可選，不建議修改
```

設備的序列號可以是地址加通訊埠號，也可以是非地址的任意字符串，可以通過 `adb devices` 命令獲取。更多資訊參見模擬器支援文件。
...
使用之前請先閱讀 [Mac模擬器支援](https://maa.plus/docs/1.4-Mac模擬器支援.html#✅-playcover-原生執行最流暢🚀)。
...
注意，如果你使用 PlayTools 連接，`touch_mode` 字段將被忽略並被設定為 `MacPlayTools`。
...
任務文件位於 `$MAA_CONFIG_DIR/tasks` 目錄下。 每一個任務文件都是一個任務列表，每一個子任務是一個 MAA 任務，其中包含任務類型和參數，可用的任務及其參數參見[集成文件](https://maa.plus/docs/3.1-集成文件.html#asstappendtask)。
...
filename = "normal.json" # 自定義的基建計劃的文件名應該位於 `$MAA_CONFIG_DIR/infrast`
...
這裡的 `condition` 字段用於確定哪一個變體應該被使用，而匹配的變體的 `params` 字段將會被合併到主任務的 `params` 字段中。

**注意**：這個 CLI 不會讀取基建計劃文件中的任何內容，包括基建計劃文件中定義的時間段，所以你必須在 `condition` 字段中定義時間段來在不同的時間執行不同的基建計劃。
