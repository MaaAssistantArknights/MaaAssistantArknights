    ...
    "plans": [
        {
            ...
            "period": [             // 換班時間段，可選
                                    // 若當前時間在該區間內，則自動選擇該計劃（整個 json 文件中可能包含多個計劃）
                                    // core 不處理該字段，若您使用接口集成 maa，請自行實現該邏輯
                [
            ...
            "rooms": {              // 房間信息，必選
                                    ...
                                    // 缺少某個則該設施使用預設算法進行換班。
                                    // 若想不對某個房間換班請使用 skip 字段，或直接在軟體 任務設定 - 基建換班 - 常規設定 中取消改設施的勾選
                "control": [
                    {
                        "operators": [
                ...
                "manufacture": [
                    ...
                    {
                        "skip": true    // 是否跳過當前房間（數組序號對應），可選，預設 false
                    ...
                    },
                    {
                        ...
                        "autofill": true,   // 使用原先的算法，自動填充剩下的位置，可選，預設 false
                                        ...
                                        // 注意可能和後面自定義的幹員產生沖突，比如把後面需要的幹員拿到這裡用了，請謹慎使用，或將 autofill 的房間順序放到最後
                    ...
                    },
                    {
                        ...
                        "candidates": [ // 備選幹員，可選。這裡面的有誰用誰，選滿為止
                                        // 與 autofill=true 不兼容，即該數組不為空時，autofill 需要為 false
                            "星源",
                            "白面鸮",
                            "赫默"
                        ]
