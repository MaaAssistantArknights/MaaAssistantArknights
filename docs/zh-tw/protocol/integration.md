...
- **復刻 SideStory 活動**：輸入 `SSReopen-<關卡前綴>` ，可一次性刷完 XX-1 ~ XX-9 關，如 `SSReopen-IC`。
  ...
  `-1`：停用自動切換連戰。
  <br>
  `0`：自動切換為當前可用的最大連戰次數（若理智不足以進行最大連戰，則選擇最低可用次數）。
  <br>
  `1` ~ `10`：指定固定的連戰次數。
  <br>
  ::: info 伺服器差異
  Core 根據遊戲內連戰列表樣式動態辨識可選次數，不硬編碼上限。輸入驗證取決於資源版本：
  <br>
  國服於 2026/8/1 更新後採用新列表樣式，接受 `-1~10`；外服仍為舊樣式，僅接受 `-1~6`，傳入更大的值會被拒絕。外服預計約半年後跟進，届時上限自動生效。
  :::  
  :::  
