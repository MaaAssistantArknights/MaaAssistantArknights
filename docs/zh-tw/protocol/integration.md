...
@type array<object>
@optional
開局幹員列表。每項為 `{ "name": 幹員名稱, "use_support": 是否借助戰 }`，幹員名稱同樣僅支援**中文名稱**（不分遊戲伺服器）；按陣列順序對應開局第 1、2、3 次招募，三個位置的幹員職業不能相同。某位置未招募到指定幹員（自有與助戰均未出現）時，該次招募按預設優先順序進行；與 `core_char` 同時傳入時以本欄位為準。  
:::  
::: field use_support  
@type boolean
@default false
