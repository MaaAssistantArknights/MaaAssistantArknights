...
```json
{
    ...
    "actions": [
        // 戰鬥中的操作。有序，執行完前一個才會去執行下一個。必填
        {
            ...
            //      "rect" 與 "location" 必須二選一，同時填寫或都不填寫會報錯並跳過該步
            // "滑動" 從 "begin" 矩形滑動到 "end" 矩形，直接下發底層滑動指令
            // 目前下面五個條件是且的關係，即 &&
            ...
            // 0 - 不啟用； 1 - 上； 2 - 下； 3 - 左； 4 - 右
            // 主滑動結束後會追加一小段指定方向的補償滑動，用於消除鬆手後主滑動方向的慣性
            // 例如左右滑動列表後追加一段垂直方向的小滑動，可避免列表鬆手後繼續滑過頭
            "slope_in": 10, // 滑動起始斜率。選填，預設為 10；以 ×10 的整數存儲，10 即 1.0
            "slope_out": 10, // 滑動結束斜率。選填，預設為 10；以 ×10 的整數存儲，10 即 1.0
