...
```json
{
    ...
    "actions": [
        // 戰鬥中的操作。有序，執行完前一個才會去執行下一個。必選
        {
            ...
            // "部署"   |  "技能"  |  "撤退"   | "二倍速"   |  "子彈時間"  |  "技能用法"   | "列印"  |  "擺完掛機" | "移動鏡頭" | "重設全局計時器"
            // 中英文皆可，效果相同
            ...
            // "列印" 界面不顯示這條步驟，僅用於輸出 doc 裡的內容（用來做字幕之類的）
            // "擺完掛機" 僅使用 "好了就用" 的技能，其他什麼都不做，直到戰鬥結束
            // "移動鏡頭" 用於 “引航者試煉” 模式，還需要填寫 distance 欄位
            // "重設全局計時器" 重設全局計時器，請參考 “time_elapsed” 條件
            // 目前下面五個條件是且的關係，即 &&
            ...
            // 注意是從上一次執行 type:ResetTimer 的 action 開始計算的
            // 使用前必須執行過 type:ResetTimer 的 action 重設計時器，不然會卡住
            // TODO: 其他條件
            ...
            // 注意 "移動鏡頭" 時是識別不到正在站場中的，需要用 sleep 完整覆蓋整個移動動畫
