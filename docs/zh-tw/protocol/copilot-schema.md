    ...
    "actions": [                    // 戰鬥中的操作。有序，執行完前一個才會去執行下一個。必選
        {
            "type": "部署",         // 操作類型，可選，預設為 "Deploy"
                                    ...
                                    // "部署"   |  "技能"  |  "撤退"   | "二倍速"   |  "子彈時間"  |  "技能用法"   | "打印"  |  "擺完掛機" | "移動鏡頭" | "重置全局计时器"
                                    // 中英文皆可，效果相同
                                    ...
                                    // "ResetTimer" — 重置全域計時器，請參考 “time_elapsed” 條件。
            ...
            "time_elapsed": 1000,   // 以毫秒為單位的全域計時條件。若未達指定時間，會持續等待。可選，預設為 0，表示立即執行
                                    // 注意：此時間是從上一次執行 type:ResetTimer 動作開始計算的
                                    // 使用前必須先執行過 type:ResetTimer 動作來重置計時器，否則會卡住
            ...
            "skip_if_not_ready": false, // 僅在 type 為 "Skill" 時有效。允許在技能不可用時跳過當前行動，主要用於關閉未完成的彈藥技能。可選，預設為 false。
