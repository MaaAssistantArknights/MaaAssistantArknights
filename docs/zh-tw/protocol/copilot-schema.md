...
```json
{
    ...
    "actions": [
        // 戰鬥中的操作。有序，執行完前一個才會去執行下一個。必填
        {
            ...
            // "部署"    |  "技能"  |  "撤退"    | "二倍速"   |  "子彈時間"  |  "技能用法"   | "列印"  |  "擺完掛機" | "移動鏡頭" | "重置全域計時器" | "點擊" | "滑動" | "設定單位座標"
            // 中英文皆可，效果相同
            ...
            // "設定單位座標" 設定某戰場單位的格子座標："name" 與 "location" 均必填，任一缺失會導致作業載入失敗
            //      用於幹員位移後修正位置，使後續按 "name" 的 "技能" / "撤退" / "技能用法" 動作落到新格子；
            //      也可用於登記場上裝置、設施等非幹員單位，配合 "技能用法" 與 "擺完掛機" 實現裝置技能自動開啟；
            ...
            //                         // 0 - 且； 1 - 或
            "name": "棘刺", // 幹員名稱 或 群組名稱， type 為 "部署" 時必填，為 "技能" | "撤退" 時選填，為 "設定單位座標" 時必填
            "role": "guard", // 幹員職業。選填，用於區分同名幹員，填英文職業名，大小寫不限
            "location": [
                5,
                5
            ...
            // type 為 "設定單位座標" 時必填，將該 "name" 單位的位置設定為此格子，座標為任意合法格子，包括 [0, 0]
