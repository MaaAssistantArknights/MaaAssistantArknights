...
- 界園肉鴿 `JieGarden/` 下的特殊檔案：
  ...
  - `map.json`：界園肉鴿地圖節點的範本圖片資訊。

- 黑流樹海肉鴿 `BlackFlow/` 下的特殊檔案：
...
1. 現有的群組介紹
   ...
   | **_群奶_**                | 範圍縱深 < 4，可治療身後       | 醫療、輔助            | 夜鶯、白面鴞、濁心斯卡蒂                                         |
           ...
           "opers": [                                        // 應該包括哪些幹員，有先後順序，代表部署優先度
               {
                   ...
                   "role": "Warrior",                        // 幹員職業，選填。存在同名幹員時用於區分職業，填英文職業名，大小寫不限
                   ...
                   "is_key": true,                           // true 為 key（關鍵）幹員，false 或省略為非 key 幹員。在陣容完備偵測未通過時，僅招募 key 幹員與 0 希望幹員，保留希望
                   ...
                   "is_alternate": false,                    // true 為後備幹員（允許重複招募，劃到後備幹員時不再往右劃動），false 或省略為普通幹員
                   ...
                   "collection_priority_offsets": [          // 依照目前擁有的藏品調控招募優先級
                       {
                           "collection": "古旧的钱币",        // 藏品名稱
                           "offset": 100                     // 擁有該藏品後對招募優先級的調整，不寫預設為 0
                       }
                   ]
               },
               ...
...
2. 部署格子黑名單
   ...
   ```json5
       ...
       "blacklist_location_Doc": "這裡是用法範例，並非指「蓄水池」這張地圖需要停用這兩個點位",
...
{
    "theme": "Sami",                              // 肉鴿主題
    "mode": [1, 4],                               // 限定該檔案在哪些模式下生效，選填；不寫則對所有模式生效
    "stage": [                                    // 不期而遇類事件
        {
            "name": "低地市集",                    // 不期而遇事件名稱
            ...
            "choices": [                          // 對特定選項的額外要求
                {
                    "name": "选择碎草药",          // 選項名稱
                    "choose": 1,                  // 滿足下方條件時優先選擇的選項序號，選填；不寫時該條不生效
                    "requirements": [             // 生效條件，選填
                        {
                            "name": "Vision",     // 條件類型，目前僅支援 Vision（燈火值）
                            "value": "3",         // 要求的數字
                            "type": ">"           // 是大於還是小於（還可以是 =，這裡表示燈火大於 3 時該條 choose 才生效）
                        }
                    ]
                },
                {
...
- `option_text`：字串陣列，事件各選項的文字，用於輔助辨識匹配
- `next_event`：字串，該事件處理完後要跳轉到的下一個事件名稱
- `fallback_choices`：形如 `[[實際選項數, 優先選擇], ...]` 的陣列，當實際辨識到的選項數與 `option_num` 不一致時，按實際選項數匹配並選擇對應的選項
...
```json5
{
    ...
    "priority": [                                                    // 優先度，清單順序即為優先級，愈靠前者愈優先購買。
...

        {
            ...
            "promotion": 2,                                          // 不為 0 時，隊伍中存在待進階幹員才會購買，預設為 0，不作要求
            "promotion_rarity": 6,                                   // 統計待進階幹員的稀有度上限，只統計稀有度低於該值的幹員，選填，預設為 6（即不限制）
