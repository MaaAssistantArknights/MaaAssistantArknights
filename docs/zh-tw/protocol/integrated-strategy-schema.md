           ...
           "opers": [                                        // 應該包括哪些幹員，有先後順序，代表部署優先度
               {
                   ...
                   "is_key": true,                           // true 為 key（關鍵）幹員，false 或省略為非 key 幹員。在陣容完備偵測未通過時，僅招募 key 幹員與 0 希望幹員，儲存希望
...
- 在選關介面點擊螢幕中上方展開坍縮狀態列位，以下稱為 Panel Check；
...
- 若 OCR 辨識出的通寶名稱在設定檔案中未找到，MAA 會將偵錯影像儲存至 `debug/roguelike/coppers/unknown` 資料夾，以供排除原因。
