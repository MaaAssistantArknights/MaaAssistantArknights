---
order: 3
icon: ph:question-fill
---

# 常見問題

如果是第一次使用 MAA，請先閱讀 [新手上路](./newbie.md)。

::: warning

若 MAA 在某次更新後無法執行，或是因為錯誤視窗而導引至此，通常是因為執行庫未更新所致。  
執行庫問題是最常見的錯誤原因，且經常被忽略，因此我們將此說明置頂。

MAA 已採用自包含部署方式，內建了 .NET 執行環境，但仍需要 Visual C++ 執行庫。

請執行 MAA 目錄下的 `DependencySetup_依赖库安装.bat`，或在終端機中執行以下指令：

```sh
winget install "Microsoft.VCRedist.2015+.x64" --override "/repair /passive /norestart" --force --uninstall-previous --accept-package-agreements
```

也可以手動下載並安裝以下執行庫來解決問題：

- [Visual C++ 可再發行套件](https://aka.ms/vc14/vc_redist.x64.exe)

:::

## 軟體無法執行 / 閃退 / 出錯

### 下載與安裝問題

- 完整的 MAA 壓縮檔命名格式為 "MAA-`版本`-`平台`-`架構`.zip"，其餘檔案皆為無法單獨執行的組成元件，請務必確認下載檔案是否完整。
  多數情況下，您需要下載 x64 架構的版本，即 `MAA-*-win-x64.zip`，而非 `MAA-*-win-arm64.zip`。
- 若自動更新後發現功能缺失或無法使用，可能是更新過程中出現錯誤。更新失敗時，MAA 會在下次啟動時提示更新失敗並提供自動修復（重新下載完整安裝檔並在重新啟動後安裝）；若曾忽略該提示，安裝可能處於半更新狀態，建議重新下載完整程式壓縮檔並再次解壓縮。  
  解壓縮後，將舊 `MAA` 資料夾中的 `config`、`data` 與 `debug` 資料夾複製到新解壓縮後的 `MAA` 資料夾中，以保留原有設定與日誌；`cache`（快取）與 `achievement`（通關截圖）資料夾可選擇保留。

### 執行庫問題

請點擊網頁右下角的 ↑ （向上箭頭）回到頂端並查看說明。

### 系統問題

- MAA 不支援 32 位元作業系統，亦不支援 Windows 7 / 8 / 8.1。
- 以上執行庫安裝均需要依賴元件存儲服務（CBS、TrustedInstaller/TiWorker、WinSxS）。
  若該服務受損，將無法正常安裝。

我們無法提供除重灌系統以外的修復建議，請避免使用過度精簡的系統版本或過舊的作業系統。

#### Windows N/KN

若您使用的是 Windows N / KN（歐洲或韓國版本），需額外安裝 [Media Feature Pack](https://support.microsoft.com/zh-tw/topic/c1c6fffa-d052-8338-7a79-a4bb980a700a)。

#### Windows 7 / 8 / 8.1

由於 MAA 依賴的執行庫和系統元件要求 Windows 10 及以上版本，MAA 不再支援 Windows 7 / 8 / 8.1 系統。

### 啟動時提示 ｢安裝檔案缺失｣

MAA 啟動時會對照內附的檔案清單（filelist.txt）檢查安裝檔案是否缺失。若彈出此提示，說明部分檔案被安全軟體攔截、刪除，或被手動刪除，對應功能（如小遊戲等任務）可能無法執行：

- 若為安全軟體所致，請前往其隔離區找回並還原被刪除的檔案，並將 MAA 安裝目錄加入白名單。
- 也可在彈窗中選擇自動修復（重新下載完整安裝檔並在重新啟動後還原），或按彈窗內的說明手動重灌。

### DLL 注入問題

MAA 啟動時彈出悲報，或出現閃退、介面算繪異常，通常是 Nahimic 等第三方軟體向 MAA 注入了 DLL 導致的。可透過以下任一方法解決：

**方法一：透過 Windows 安全中心阻止 DLL 注入（推薦）**

1. 開啟 **Windows 安全中心** → **應用程式與瀏覽器控制** → **入侵防護** → **入侵防護設定** → **程式設定**
2. 點擊 **新增程式進行自訂**，選擇 **「依程式名稱新增」** 並輸入 `MAA.exe`（也可選擇 **「選擇確切的檔案路徑」** 後定位到 MAA 安裝目錄下的 `MAA.exe`）
3. 在 MAA 的設定清單中找到 **「停用擴充點」**，勾選 **「覆寫系統設定」** 並開啟開關
4. 點擊 **「套用」** 儲存，然後重新啟動 MAA

該設定從系統層面阻止第三方軟體向 MAA 注入 DLL，不會影響其他程式的正常執行。

**方法二：重新命名注入的 DLL**

1. 在彈窗給出的路徑下找到對應的 DLL 檔案（若看不到副檔名或該檔案，需先在檔案總管頂端 **檢視 → 顯示** 中勾選 **「副檔名」** 和 **「隱藏的項目」**）
2. 右鍵該檔案 → **重新命名**，將副檔名 `.dll` 改為 `.dll1` 等無效副檔名

::: warning
該方法可能導致相關程式出錯，請謹慎使用。想了解問題原因可查看[相關動態](https://t.bilibili.com/1133690423484612615)和 [blog](https://blog.walterlv.com/post/wpf-renders-wrong-because-of-nahimicosd.html)。
:::

**方法三：處理相關軟體**

嘗試將 MAA 加入相關軟體的排除名單，或直接解除安裝相關軟體後檢查問題是否仍然存在。

## 連線錯誤

### 確認 ADB 與連線位址設定

請參閱 [連線設定](./connection.md) 檢查相關設定。

### 關閉殘留的 ADB 處理程序

關閉 MAA 後，請檢查 `工作管理員 - 詳細資料` 標籤中是否仍有 `adb` 相關處理程序。若有，請將其結束後再重新嘗試連線。

### 多個 ADB 版本衝突問題

當環境中存在多個不同版本的 ADB 時，新啟動的處理程序可能會中斷舊有的處理程序。  
若您需要同時執行多個工具（如 Android Studio、Alas、手機助手等），請確保所有工具使用的 ADB 版本一致。

### 避免加速器干擾

部分加速器在啟動或停止加速後，需重新啟動 MAA、ADB 與模擬器才能正常連線。

MuMu 模擬器與 UU 加速器同時使用的設定可參閱 [官方說明文件](https://mumu.163.com/help/20240321/35047_1144608.html)。

### 重新啟動電腦

重新啟動電腦能解決 97% 的問題。（確信

### 更換模擬器

若上述方法皆無效，請參閱 [模擬器與設備支援](./device/)。

## 連線正常，但無動作

部分模擬器內建的 ADB 版本過於老舊，無法支援 `Minitouch` 或 `MaaTouch`。

請嘗試以系統管理員權限開啟 MAA，關閉模擬器並重啟 MAA 後，點擊 `MAA 設定 - 連線設定 - 強制替換 ADB`。

模擬器更新後可能會重新覆蓋 ADB 檔案。若問題再次出現，請重新操作替換，或參閱 [其他 ADB 解決方案](./connection.md#google-adb)。

## 連線正常，但操作異常、卡頓或頻繁出錯

- 若開啟了 `螢幕 UI 調整`，請嘗試將其歸零。
- 若遊玩非官服（外服）客戶端，請確認 `設定 - 執行設定 - 客戶端類型` 已正確切換。外服部分功能可能未完全適配，請參考對應的外服使用文件。
- 執行自動肉鴿時，請參閱 [自動肉鴿文件](./introduction/integrated-strategy.md)，並確認 `自動肉鴿` → `肉鴿主題` 選擇正確。
- 若自動戰鬥頻繁暫停且不部署幹員，請關閉 `設定 - 執行設定` 中的 `暫停下幹員`。
- 自動編隊若無法辨識幹員，請取消該幹員的「特別關注」。
- 使用 `ADB Input` 觸控模式時，操作較緩慢為正常現象。
- MuMu 模擬器使用者請勿將 `顯存使用策略` 設定為 `資源佔用更小`。

### 提示截圖耗時較長或過長

- MAA 目前支援 `RawByNc`、`RawWithGzip`、`Encode` 三種基於 ADB 的截圖方式。當執行任務的平均截圖耗時大於 400 或 800 毫秒時，會顯示一次提示訊息（單次任務僅會顯示一次）。
- 在 `設定 - 連線設定` 中會顯示最近 30 次截圖耗時的 最小值 / 平均值 / 最大值，每 10 次截圖更新一次。
- 自動戰鬥類功能（如自動肉鴿）受截圖耗時的影響較大。
- 此耗時與 MAA 無關，通常受電腦效能、背景處理程序佔用或模擬器設定影響；可嘗試清理背景處理程序、更換模擬器或提升硬體配置。

## 系統管理員權限相關說明

MAA 在一般情況下，無需以 UAC（系統管理員身分）執行即可使用所有功能。目前與權限相關的功能主要包括：

1. `自動偵測連線`：當目標模擬器以系統管理員身份執行時，MAA 也需要對應權限。
2. `任務完成後` 中的 `關閉 模擬器`：當目標模擬器以系統管理員身分執行時，需要對應權限。
3. `開機自動啟動 MAA`：無法在系統管理員身份下設定開機自動啟動。
4. 當 MAA 被錯誤地解壓縮到需要系統管理員權限才能寫入的路徑時（例如 `C:\`、`C:\Program Files\`）。

有些案例反映，若關閉了 Windows 的 UAC 功能，系統會出現「即使沒有點選右鍵選擇以系統管理員身分執行，也會自動提升權限啟動」的問題，建議保持開啟狀態以避免非預期的權限提升行為。

## 下載中斷或提示「權限驗證失敗」

請使用瀏覽器或 IDM、FDM 等專門的下載工具，**不要用 ↑↓ 迅雷！**
