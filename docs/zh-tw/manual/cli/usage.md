...
完成 MaaCore 的安裝與配置後，您就可以執行任務了。maa-cli 支援兩種類型的任務：預定義任務和自訂任務。
...
由於 MAA 支援的任務繁多，maa-cli 無法提供所有任務的預定義選項。除此之外，您可能需要像上述的例子一樣執行多個任務。為了解決這個問題，maa-cli 提供了自訂任務的功能。自訂任務能夠組合不同的任務，並且更精確地控制每個任務的參數以及執行順序。此外，自訂任務支援條件判斷，可以根據條件來決定是否執行某個任務，或者以何種參數執行某個任務。這可以用於自動化您的日常任務。自訂任務透過配置檔案定義，具體配置檔案的位置和編寫方式請參考 [自訂任務文件][custom-task]。在編寫好配置檔案後，您可以透過 `maa run <task>` 來執行自訂任務，這裡的 `<task>` 是一個自訂任務檔案名稱，不包括副檔名。

### 任務總結

不管是預定義任務還是自訂任務，maa-cli 都會在任務執行結束後輸出任務的總結資訊，
