...
歡迎造訪作業分享站 [prts.plus](https://prts.plus) 使用及分享作業檔案。
...
- 本功能需要在有 `開始行動` 按鈕的編隊選擇介面開始執行。
  之後在 MAA 左側上方的方框中 `匯入本地 JSON 作業檔案` 或 `寫作業站神秘代碼` 即可匯入作業。
...
- 請使用 [作業編輯器](https://prts.plus/create) 製作，可參考 [戰鬥流程協定](../../protocol/copilot-schema.md)。
- 地圖座標獲取：
...
- 支援演習模式。
- 建議在作業描述裡填上您的署名（作者名）、參考的攻略影片連結、其他想說的話等等。
