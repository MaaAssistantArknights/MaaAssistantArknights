...
- 可使用 `多作業模式` 功能進行同一區域關卡的自動連續戰鬥。
  - 作業列表下方三個按鈕從左到右依序為 `批次匯入`、`加入`、`清除`。
    `加入` 左鍵為加入普通難度關卡，右鍵為加入突襲難度關卡；`清除` 左鍵為清除所有關卡，右鍵為清除未勾選關卡，
  - 匯入作業後，作業列表下方會出現關卡名稱，確認正確後再新增該關卡。列表中的關卡可以拖曳調整順序，並勾選是否執行。
  - 開啟本功能後，改為在**關卡所在的地圖介面**開始自動戰鬥。在理智不足 / 戰鬥失敗 / 非三星結算時，將停止自動戰鬥佇列。
