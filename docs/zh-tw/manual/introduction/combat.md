...
- 進階設定中的 `剿滅僅使用全權委託` 預設關閉。關閉時維持原有行為：優先嘗試全權委託，全權委託不可用但普通代理可用時會繼續執行普通代理；普通代理也不可用時任務失敗。
- 啟用 `剿滅僅使用全權委託` 後，每次開始剿滅前都會檢查是否可用全權委託。若代理券已用盡或全權委託不可用，目前理智作戰會正常結束，並繼續執行下一個任務，不會回退到普通代理。
