...
- `吃理智藥` + `吃源石` 和 `指定次數`、`指定材料` 三個選項為為「或」邏輯關係，  
  ...
  - `指定材料` 指定刷多少個指定材料（例如「取得 5 個固源岩後停止」），支援兩種計數模式：
    ...
    - `目標庫存`：參考 [倉庫辨識](./tools.md#倉庫辨識) 中保存的庫存資料，只補到設定庫存為止。需要先通過 [更新資料](./user-data-update.md) 或 [倉庫辨識](./tools.md#倉庫辨識) 取得庫存。此模式由 UI 實作（MaaCore 僅支援掉落數量模式，UI 根據庫存快取計算缺口後轉換為掉落數量）。
