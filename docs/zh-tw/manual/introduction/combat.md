...
- 也可以在 `設定 - 理智作戰 - 進階設定` 中啟用 `手動輸入關卡名稱` 手動輸入關卡編號。目前支援導航的關卡有：
...
範例：關卡備選選擇了 `龍門幣-6/5`、`1-7` 與 `經驗-6/5`：
...
在 `刷理智` 任務結束後啟動，不受 `吃理智藥`、`吃源石`、`指定次數`、`指定材料`、`代理倍率` 的控制，刷完目前剩餘理智即結束。

- 適用於在 `關卡選擇` 關卡理智不足後，繼續前往 `剩餘理智` 關卡清理剩餘的「零散」理智（如前往 `1-7`）。
- 亦適用於在代理倍率設定過高而理智不足時自動以單次連戰刷光理智收尾（設定 `1-7` 連戰 6 次，但只有 30 理智，於是自動轉為刷 5 次不連戰的 `1-7`）。
- 若剩餘理智仍然不足則會結束任務（如少於 6 理智）。
- 如果剩餘理智選擇關卡為未開放關卡，則理智作戰任務出錯。
