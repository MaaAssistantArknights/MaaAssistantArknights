...
::: info UI 專屬功能
本頁部分功能由 UI 層實作（如目標庫存、智慧跳過、週計畫、多任務排序等），詳見[新手上路](../newbie.md#關於本文檔)中的說明。
:::
...
- 「根據活動排程智慧跳過」與「週計畫」位於 `任務設定` - `刷作戰` - `進階設定`，兩個選項獨立生效。這些功能由 UI 實作。
- 「根據活動排程智慧跳過」：先檢查目前遊戲日是否有 SideStory 活動；若有，且連續或重疊的後續活動會在本週結束前終止，則跳過目前的刷作戰任務，以便在活動結束後的任意一天執行剿滅作戰。若 SideStory 活動連續覆蓋本週剩餘時間直至週日結束，則不會跳過。
- 啟用「週計畫」後，可勾選週日～週六中哪些**遊戲日**執行該刷作戰任務。
