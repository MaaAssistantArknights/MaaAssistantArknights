...
::: tip
사용 중인 에뮬레이터가 지원하는지, 확인하려면 다음 문서를 참조하세요. [Windows 지원 목록](./devices/windows.md)
:::
...
[Windows 지원 목록](./devices/windows.md)을 참고하세요.
...
- 통합전략 기능을 사용할 경우, [사용자 설명서](./introduction.md)를 참조하고 `통합전략` - `테마`에서 올바른 테마를 선택하세요
