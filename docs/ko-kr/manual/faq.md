...
- 자동 업데이트 후 기능이 누락되었거나 작동하지 않는 경우, 업데이트 과정에서 문제가 발생했을 수 있습니다. 전체 설치 패키지를 다시 다운로드하고 압축을 해제하세요. 압축 해제 후, 이전 `MAA` 폴더에 있는 `config`, `data`, `debug` 폴더를 새로 압축 해제된 `MAA` 폴더로 복사하여 기존 설정과 로그를 유지하세요. `cache`(캐시)와 `achievement`(클리어 스크린샷) 폴더는 필요에 따라 유지할 수 있습니다.
