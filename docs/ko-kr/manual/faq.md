...
- 자동 업데이트 후 기능이 누락되었거나 작동하지 않는 경우, 업데이트 과정에서 문제가 발생했을 수 있습니다. 업데이트가 실패하면 MAA는 다음 시작 시 업데이트 실패 알림을 표시하고 자동 복구(전체 패키지를 다시 다운로드하여 다시 시작 후 설치)를 제안합니다. 해당 알림을 무시했다면 설치가 불완전한 상태일 수 있으니 전체 설치 패키지를 다시 다운로드하고 압축을 해제하세요. 압축 해제 후, 이전 `MAA` 폴더에 있는 `config`, `data`, `debug` 폴더를 새로 압축 해제된 `MAA` 폴더로 복사하여 기존 설정과 로그를 유지하세요. `cache`(캐시)와 `achievement`(클리어 스크린샷) 폴더는 필요에 따라 유지할 수 있습니다.
...
### 시작 시 ｢설치 파일 누락｣ 알림이 표시됨

MAA는 시작 시 설치 패키지에 포함된 파일 목록(filelist.txt)과 대조하여 설치 파일 누락 여부를 확인합니다. 이 알림이 표시되면 일부 파일이 보안 소프트웨어에 의해 차단·삭제되었거나 수동으로 삭제되어 해당 기능(미니게임 등의 작업)이 동작하지 않을 수 있습니다:

- 보안 소프트웨어가 원인이라면 해당 소프트웨어의 격리소에서 삭제된 파일을 복원하고, MAA 설치 폴더를 허용 목록에 추가해 주세요.
- 알림 창에서 자동 복구(전체 패키지를 다시 다운로드하고 재시작 후 복원)를 선택하거나, 알림 창의 안내에 따라 수동으로 재설치할 수도 있습니다.

### DLL 인젝션 문제

MAA 시작 시 호환성 경고가 표시되거나, 충돌·UI 렌더링 문제가 발생하는 것은 일반적으로 Nahimic 등 타사 소프트웨어가 MAA에 DLL을 주입했기 때문입니다. 다음 방법 중 하나로 해결할 수 있습니다:

**방법 1: Windows 보안을 통해 DLL 인젝션 차단 (권장)**

1. **Windows 보안** → **앱 및 브라우저 제어** → **Exploit Protection** → **Exploit Protection 설정** → **프로그램 설정** 을 엽니다
2. **사용자 지정할 프로그램 추가** 를 클릭하고, **"프로그램 이름으로 추가"** 를 선택하여 `MAA.exe` 를 입력합니다 (또는 **"정확한 파일 경로 선택"** 을 선택하여 MAA 설치 디렉터리의 `MAA.exe` 를 지정)
3. MAA 설정 목록에서 **"확장 지점 사용 안 함"** 을 찾아, **"시스템 설정 재정의"** 에 체크하고 켭니다
4. **"적용"** 을 클릭하여 저장한 뒤 MAA를 다시 시작합니다

이 설정은 시스템 수준에서 타사 소프트웨어가 MAA에 DLL을 주입하는 것을 차단하며, 다른 프로그램에는 영향을 주지 않습니다.

**방법 2: 주입된 DLL 이름 바꾸기**

1. 경고에 표시된 경로에서 해당 DLL 파일을 찾습니다 (파일 확장명이나 파일 자체가 보이지 않으면, 탐색기 상단의 **보기 → 표시** 에서 **"파일 확장명"** 과 **"숨겨진 항목"** 을 활성화하세요)
2. 파일을 우클릭 → **이름 바꾸기** 하여 확장명 `.dll` 을 `.dll1` 등 무효한 확장명으로 변경합니다

::: warning
이 방법은 관련 소프트웨어에 문제를 일으킬 수 있습니다. 신중하게 사용하세요. 원인에 대한 자세한 내용은 [관련 게시물](https://t.bilibili.com/1133690423484612615)과 [blog](https://blog.walterlv.com/post/wpf-renders-wrong-because-of-nahimicosd.html)을 참조하세요.
:::

**방법 3: 관련 소프트웨어 처리**

관련 소프트웨어의 제외 목록에 MAA를 추가하거나, 관련 소프트웨어를 제거한 후 문제가 해결되었는지 확인하세요.
...
- 국가 별 클라이언트를 플레이하는 경우 먼저 `설정` - `실행 설정` - `클라이언트 종류`에서 클라이언트 버전을 선택하세요. 해외 클라이언트의 경우 일부 기능이 완전히 호환되지 않을 수 있으므로 해당 해외 클라이언트 사용 설명서를 참조하세요.
- 통합 전략 기능을 사용할 경우, [사용자 설명서](./introduction/integrated-strategy.md)를 참조하고 `통합 전략` - `테마`에서 올바른 테마를 선택하세요
- 자동지휘가 자주 일시 정지되고 오퍼레이터가 배치되지 않으면 `설정` - `실행 설정`에서 `일시정지 후 배치`를 비활성화하세요.
...
2. `완료 후` 의 `에뮬레이터를 종료`：목표 에뮬레이터가 관리자 권한으로 실행될 때 관리자 권한이 필요합니다.
