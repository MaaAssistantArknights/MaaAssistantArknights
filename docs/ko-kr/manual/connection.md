...
중국 공식판 또는 Ark Edition MuMu 12 V4.0.0 및 이후 버전만 지원합니다. 또는 국제판 MuMu 12 V4.1.26 이상의 버전을 사용해야 합니다. <!-- V3.8.13 -->

1. 설정 → 연결 설정에서 `MuMu 스크린샷 향상 모드 활성화`를 체크합니다. 체크할 경우 MAA는 레지스트리를 통해 설치 경로를 자동으로 입력하려고 시도합니다.

2. MuMu 에뮬레이터 경로에 `MuMuPlayer-12.0` 또는 `YXArkNights-12.0` 또는 `MuMuPlayerGlobal-12.0` 폴더의 경로를 입력합니다. 예: `C:\Program Files\Netease\MuMuPlayer-12.0`.
