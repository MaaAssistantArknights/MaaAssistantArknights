...
- 응접실은 매 회전마다 단서를 발송하려 시도합니다. 공식(Official, Bilibili) 클라이언트는 2025-12-05에 '중복 단서를 한 번에 보내기' 기능을 추가했습니다. 해당 버튼이 감지되면 MAA가 자동으로 클릭하여 중복 단서를 일괄 전송합니다. EN/JP/KR은 약 6개월 내 지원될 예정이며, TW는 약 1년 후 지원될 예정입니다(대만 서버는 진행 상황에 따라 더 빨리 지원될 수 있습니다)。
