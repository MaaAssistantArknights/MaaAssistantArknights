...
- `목표 재고` 모드는 [재고 유지](./depot-maintain.md)와 시작 판정을 공유합니다: 작업 시작 시 최신 재고로 부족분을 다시 계산하여 이미 달성했으면 바로 건너뜁니다. 또한 이 큐 실행 중 이전 전투가 보고한 이성이 스테이지 소모 미만이고 이 작업에 회복제/오리지늄 한도가 없는 경우(만료 임박 회복제 소진이 확인된 경우)에도 마찬가지로 건너뛰며, 어느 쪽이든 스테이지에 진입하지 않습니다.
