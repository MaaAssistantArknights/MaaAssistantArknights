...
- `이성 사용`의 일반 설정에있는 `이성 회복제 사용 + 순오리지늄 사용`과 `횟수 제한` 및 `드롭 제한` 세 가지 옵션은 하나만 만족해도 종료가 됩니다.
  ...
  - `드롭 제한` 은 특정 재화를 몇개 획득할 지 정합니다. (5개의 장치 획득 후 정지) 두 가지 카운트 모드를 지원합니다:
    ...
    - `목표 재고`: [창고 인식](./tools.md#창고-인식)에 저장된 재고 데이터를 참조하여, 설정 재고까지 보충합니다. [데이터 업데이트](./user-data-update.md) 또는 [창고 인식](./tools.md#창고-인식)으로 재고 데이터를 먼저 가져와야 합니다. 이 모드는 UI에서 구현됩니다(Core는 드롭 수량 모드만 지원).
