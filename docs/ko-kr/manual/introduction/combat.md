...
- **숫자 모드** (1~6):
  ...
  - 현재 이성이 설정된 횟수를 완료하기에 충분하지 않은 경우(예: 5회만 가능하지만 6회로 설정된 경우), 작업을 바로 종료하고 `잔여 이성` 프로세스(설정된 경우)를 시작합니다.

- **비활성 모드** (-1):
  - 게임 내 연속 전투 횟수를 조정하지 않습니다.
  - 이성이 현재 설정된 횟수를 완료하기에 충분하지 않은 경우 작업을 바로 종료하고 `잔여 이성` 프로세스(설정된 경우)를 시작합니다.
