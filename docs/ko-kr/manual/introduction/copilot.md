...
[zoot.plus](https://zoot.plus)에서 이용해 다운로드/업로드 및 제작할 수 있습니다!

::: warning
자동 전투와 관련된 모든 기능에는 60프레임 이상의 안정적인 게임 프레임 레이트가 필요합니다.
:::
...
- 이 기능을 사용하려면 전투 준비 화면에서 `작전 개시` 버튼이 있는 곳에서 시작해야 합니다.
  그런 다음 MAA의 `자동지휘` 칸에서 `JSON 파일`을 가져오거나 [zoot.plus](https://zoot.plus)와 같은 사이트에서 코드를 입력하면 됩니다.
...
- [파일 작성기](https://zoot.plus/create)를 사용할 수 있고, [전투 스키마 문서](../../protocol/copilot-schema.md)를 참조할 수도 있습니다.
- 좌표를 가져오려면:
