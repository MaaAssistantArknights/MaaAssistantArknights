...
- 동일한 지역의 스테이지를 연속적으로 전투하려면, `전투 목록` 기능을 사용할 수 있습니다.
  - `전투 목록 아래`의 세 가지 버튼은 왼쪽에서 부터 `폴더 가져오기`, `스테이지 추가`, `스테이지 지우기`입니다.  
    `스테이지 추가` 우클릭은 작전 스테이지 추가이고, `스테이지 지우기` 우클릭은 선택되지 않은 스테이지만 지우기입니다.
  - 작업을 가져온 후, 전투 목록 아래에 스테이지 이름이 표시됩니다. 올바른지 확인한 후 해당 스테이지를 추가하세요. 목록의 스테이지는 드래그하여 순서를 조정하고, 실행 여부를 체크할 수 있습니다.
