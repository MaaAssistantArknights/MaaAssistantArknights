...
- `payload`는 UTF-8로 인코딩된 JSON 텍스트이며 다음 필드를 포함합니다:
  ...
  - `event`: 이벤트 유형. 값은 다음과 같습니다
    ...
    - 전투 계열 작업에는 `sanity` 객체가 붙을 수 있습니다: 스테이지 진입 전에 인식한 이성 스냅샷(이번 실행에서 마지막으로 스테이지에 들어가기 전의 값)으로, `current` / `max`(현재 이성과 상한)와 함께 `recoverFullAt`(시간대가 포함된 ISO 8601의 전체 회복 예상 시각) 및 `recoverMinutes`(전송 시점에 계산한 남은 분)를 포함합니다. 추정 기준은 자연 회복 6분당 1포인트입니다. 인식에 실패했거나 이번 실행에서 스테이지에 진입하지 않은 경우 이 필드는 생략됩니다.
