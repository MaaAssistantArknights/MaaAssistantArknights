...
```json
{
    "TaskName": {                           // 작업 이름, @가 포함된 경우 특수 작업일 수 있으며 기본값이 다를 수 있음. 아래 특수 작업 유형 참조
        ...
        "cache": false,                     // 선택 사항, 캐시 사용 여부, 기본값 false
                                            // 활성화하면 첫 인식 성공 시 이후에는 해당 위치에서만 인식. 성능 대폭 절약 가능
                                            // 단, 인식 대상 위치가 절대 변하지 않는 경우에만 사용. 변할 수 있다면 기본값 false 유지
        ...
        "specialParams": [int, ...],        // 특정 인식기에 필요한 추가 파라미터
                                            // action이 Swipe일 때 선택 사항: [0] duration, [1] 추가 슬라이드 방향(0 비활성, 1/2/3/4는 각각 상/하/좌/우),
                                            // [2]와 [3]은 각각 스와이프 궤적의 이즈인(ease-in), 이즈아웃(ease-out) 기울기, 10배하여 입력 필요, 기본값은 둘 다 10
                                            // 정상적인 이즈인/이즈아웃을 원하면 [2]와 [3]을 각각 37, 1로 설정하는 것을 권장
        ...
        "nmsDistance": 0,                   // 선택 사항, 템플릿 매칭 다중 결과 중복 제거 반경, 단위는 픽셀, 기본값 0
                                            // 두 검출 위치의 가로세로 좌표 차가 모두 이 값 미만이면 점수가 가장 높은 것만 유지
                                            // 0이면 템플릿 짧은 변의 절반으로 계산
        ...
        "replaceFull": false,               // 선택 사항, ocrReplace 일치 시 인식 텍스트 전체를 교체할지 여부, 기본값 false
                                            // false: 일치한 부분 문자열만 교체, true: 텍스트 전체가 교체 규칙에 일치하면 전체를 해당 규칙의 교체 값으로 변경
