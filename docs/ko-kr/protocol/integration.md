...
<br>
::: info 서버 차이
Core는 게임 내 연속 전투 목록 스타일에서 OCR로 사용 가능한 횟수를 동적으로 식별하며, 상한을 하드코딩하지 않습니다. 입력 검증은 리소스 버전에 따라 다릅니다:
<br>
중국 서버는 2026/8/1 업데이트 후 새 목록 스타일을 채택하여 `-1~10`을 허용합니다. 해외 서버는 여전히 이전 스타일로 `-1~6`만 허용하며, 이를 초과하는 값은 거부됩니다. 해외 서버는 약 반년 후에 따를 예정이며, 그 시점에 상한이 자동으로 적용됩니다.
:::  
:::  
