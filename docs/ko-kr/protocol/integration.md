...
@type array<object>
@optional
시작 오퍼레이터 목록. 각 항목은 `{ "name": 오퍼레이터명, "use_support": 지원 유닛 사용 여부 }`이며, 오퍼레이터명은 마찬가지로 **중문명**만 지원(서버 무관). 배열 순서대로 시작 시 1·2·3번째 모집에 대응하며, 세 순위의 오퍼레이터는 서로 다른 직업이어야 함. 특정 순위에서 오퍼레이터를 모집하지 못한 경우(보유·지원 유닛 모두 등장하지 않은 경우) 해당 회차는 기본 우선순위로 모집됨. `core_char`와 함께 전달되면 이 필드가 우선됨  
:::  
::: field use_support  
@type boolean
@default false
