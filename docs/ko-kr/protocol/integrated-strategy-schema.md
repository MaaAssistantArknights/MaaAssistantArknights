...
1. 기존 그룹 소개
   ...
   `_奶_`는 종합적인 치유 능력을 고려합니다. 단일 메딕(4칸)이나 멀티 메딕(후방 커버 가능)의 공격 범위 구분이 필요한 경우 별도로 분류하여 사용해 주세요.

   `_高台输出_`는 순수 화력을 고려합니다. 공격 타입이나 범위 제한이 필요한 경우 `_速狙_`, `_术师_`, `_辅助_`, `_盾法_` 등을 별도로 사용해 주세요.
