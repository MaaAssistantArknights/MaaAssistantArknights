...
```json
{
    ...
    "actions": [
        // 전투 중 조작. 순서가 있으며, 이전 동작이 완료되어야 다음 동작 실행. 필수
        {
            ...
            //      "rect"와 "location"은 둘 중 하나만 지정해야 함. 둘 다 지정하거나 모두 비워 두면 파싱 오류로 작업 파일 로드에 실패함
            ...
            // type이 "Click"인 경우 "rect"와 둘 중 하나만 지정. 해당 칸을 직접 클릭. 좌표는 [0, 0]을 포함한 모든 유효한 칸 가능
            "rect": [
                100,
                100,
                50,
                50
            ], // type이 "Click"인 경우 "location"과 둘 중 하나만 지정. 720p 기준 픽셀 사각형 [x, y, w, h]
