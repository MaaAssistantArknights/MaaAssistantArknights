...
[기반 시설 스케줄링 생성 도구](https://ark.yituliu.cn/tools/schedule)
    ...
    "plans": [
        {
            ...
            "Fiammetta": {
                ...
                "target": "巫恋", // 목표 오퍼레이터, OCR을 사용하므로 해당 클라이언트 언어의 오퍼레이터명을 입력해야 함
            ...
            "rooms": {
                ...
                // 특정 방의 교대를 원하지 않으면 skip 필드를 사용하거나, 소프트웨어 '임무 설정 - 기반 시설 교대 - 일반 설정'에서 해당 시설의 체크를 해제하세요
                "control": [
                {
                    "operators": [
                        "夕", // OCR을 사용하므로 해당 클라이언트 언어의 오퍼레이터명을 입력해야 함
                        "令",
                        "凯尔希",
                        "阿米娅",
                        "玛恩纳"
                ...
                {
                    "skip": true // 현재 방을 건너뛸지 여부 (배열 순서 대응), 선택 사항, 기본값 false
                ...
                },
                {
                    ...
                    // operators가 비어 있으면, 해당 방은 완전히 기존 알고리즘을 사용하여 스케줄링합니다
                    // operators가 비어 있지 않으면, 단일 오퍼레이터 효율만 고려하며 전체 조합 효율은 고려하지 않습니다
                    // 뒤에 정의된 사용자 오퍼레이터와 충돌할 수 있으니 주의하세요. 예를 들어 뒤에 필요한 오퍼레이터를 여기서 가져다 쓸 수 있으므로 신중하게 사용하거나, autofill 방 순서를 마지막으로 배치하세요
