...
더 많은 자세한 정보는 [Copilot Schema](3.3-COPILOT_SCHEMA.md)를 참조하세요.
...
더 많은 자세한 정보는 [Copilot Schema](3.7-SECURITY_PRESENCE_SCHEMA.md)를 참조하세요.
...
- `ReclamationAlgorithm <br>`
  생명연산 (CN 클라이언트의 새로운 모드, 한섭 출시 예정 ~~할배들 번역 및 주석 요약 바람~~)
...
```jsonc
{
    ...
    "subtask": string,  // "stage" | "start" | "action"
                        ...
                        //           예: "details": { "name": "史尔特尔", "location": [ 4, 5 ], "direction": "左" }，자세한 내용은 3.3-전투_절차_프로토콜.md 참조
    "details": {
        ...
    }
}
...
```jsonc
    enum InstanceOptionKey
    {
        ...
        DeploymentWithPause = 3,    // 일시 중지(ESC키 PAUSE 컨트롤)와 함께 배치 (로그라이크, Copilot 및 보안파견용)
                                    // "1" | "0"
