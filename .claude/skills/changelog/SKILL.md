...
- 当新版本跨越次版本号（如 v6.12.x → v6.13.x），CHANGELOG 中不保留上一个次版本号（v6.12.x）的任何历史折叠块。
- 次版本号升级意味着全新的功能周期，旧版本内容对新版本用户没有参考价值。
- 同次版本号内的 patch 版（如 v6.12.1、v6.12.2）之间仍需保留历史折叠块。
- 同次版本号内的测试版（如 v6.13.0-beta.1、v6.13.0-beta.2）之间不保留历史折叠块——发布正式版时合并为单一区块（见规则 §6）。
...
- 跨次版本号时仍保留旧版本的历史折叠块，导致 CHANGELOG 累积大量不相关内容。
