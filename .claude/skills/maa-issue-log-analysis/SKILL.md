...
- **哪些正文里的 `@` 会触发 mention / 通知（需要按下面规则处理）**：出现在会被 GitHub 当**普通富文本**渲染的段落、列表、表格单元格等里的 `@用户名`、`@org/team-name`、机器人账号等；引用块（`>`）里的行文通常仍按 Markdown 解析，**同样可能**触发 mention。不要把这类 `@` 原样从历史 comment 抄进最终发帖版。
- **哪些语境里的 `@` 一般不必为「防误通知」而改掉**：落在 **GitHub 围栏代码块**（连续三个反引号起止的代码围栏）或 **行内代码**（一对反引号包裹的片段）里的 `@` 通常按字面量显示，**不会**被当成 mention；粘贴日志、配置、命令、堆栈时优先用代码块，既保真又减少误通知。若日志只能放在未格式化的正文或引用块里，其中的 `@` 仍可能被解析，需按上条处理或改写展示方式。
- **处理方式**：
...
- 若将把内容发布为 GitHub issue 评论，发帖前整稿按「GitHub 评论发布前」一节检查（含 mention 触发语境与 `@` 清理）。
- 不要只看 `gui.log` 下结论。
